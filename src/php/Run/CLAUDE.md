...
- **Uncaught runtime errors**: `ReplOutputTrait::writeReplError()` prints `CommandFacade::getRuntimeErrorReport()`, the very report `phel run` writes, and logs the full trace. The prompt owned a second formatter of its own until #3264; do not grow another one here.
