...
- `defstruct`: a field's `^{:tag <type>}` emits a typed property (`protected int $id;`); `^{:php/attr [...]}` on the struct name (class-level) or a field (property-level) emits PHP 8 attributes. `^{:php/json true}` on the struct name implements `\JsonSerializable` (emitting a `jsonSerialize()` that returns the field map) and `^{:php/stringable true}` declares `\Stringable`. A `:php` marker in the impl list opens a block of bare PHP methods (magic methods like `__invoke`/`__toString`/`__get`) emitted directly on the class with **no backing interface** — `DefStructSymbol` skips interface reflection for these and carries them as a `DefStructInterface` with an empty name, which `DefStructEmitter::interfaceNames()` filters out of the `implements` list. A custom `__invoke` must stay signature-compatible with the inherited map-lookup `__invoke(mixed $key)` (one required call arg or variadic); `DefStructSymbol::assertCompatibleInvoke()` rejects bad arity with a Phel error.
