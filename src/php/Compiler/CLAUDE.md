...
- All four route through one private `raise()`, which pins `display_errors` to `stderr` for the duration of the `trigger_error()` call (skipped when display is already off or already on stderr, so the redirect never *enables* a silenced notice). `MethodEmitter`'s `^:reference` check runs during emission, inside the emitter's `ob_start()`; under PHP CLI's default `display_errors=1` the notice text landed in that buffer and was spliced into the generated PHP, so `--warn-deprecations` failed the compile with `syntax error, unexpected token ":"` (#2827). Keep new notices on `raise()`: a diagnostic must never be able to corrupt captured output.
