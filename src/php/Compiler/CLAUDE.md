...
- At opt level >= 2, and never on a `:redef` `update` (direct linking, same gates as `CallInliner`), `InvokeSymbol` rewrites `(update m k (fn [v x ...] body) x ...)` with a literal one-arity `fn` into the `let` the runtime `update` runs, body spliced in, before analysing the arguments (`UpdateLiteralFnLowering`, #3322). The params become `let` bindings, so hygiene, nested captures and destructuring come from `LetSymbol`. In expression position the `let` is flagged `LetNode::isInlineInExpression()` and `LetEmitter` chains its bindings as assignments inside one expression instead of an IIFE, which would cost what the closure did. The body then runs in the enclosing PHP scope rather than a by-value closure, so `ClosureBarrierDetector` keeps the runtime call when it could write an enclosing local (an array write whose target mentions one, `php/ref`, a local or an offset into one handed to a PHP function or method) or `php/yield`. Statement position, `recur` in the body, rest/multi-arity/named/tagged fns and a leading `:pre`/`:post` map (even as the only form) also keep it.
