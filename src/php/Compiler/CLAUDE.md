...
- At opt level >= 2, and never on a `:redef` `update` (direct linking, same gates as `CallInliner`), `InvokeSymbol` lowers an analysed `(update m k (fn [v x ...] body) x ...)` whose fn is a literal single-arity `FnNode` into the `let` the runtime `update` runs (`UpdateLiteralFnLowering`, #3322). It works on the analysed call, so the fn body is analysed once in its own env (macros see the real `&env`, params stay untyped) and declining just builds the ordinary `CallNode`. Params become `let` bindings under fresh PHP names (`ParamRenamer` rewrites the references and maps each env to the fresh name, which is what an IIFE's `use(...)` reads), so no source-named variable is left in the caller's frame; a nested fn declines, since its captures name the param. Extra arguments must pass the same allowlist as the body: the closure captured the enclosing locals before they ran. The body's value is re-homed from return to expression position by `ExpressionContextRebuilder` (calls, leaves, collection literals, `if`; anything else becomes an IIFE `do`). In expression position the `let` is flagged `LetNode::isInlineInExpression()` and `LetEmitter` chains its bindings as assignments inside one expression instead of an IIFE. The body runs in the enclosing PHP frame rather than its own closure, so it lowers only when every node is on the `SpliceableBody` allowlist: literals, local and global reads, collection literals without reader metadata (it is evaluated too), `php/aget`, `if`/`do`/`let` (not `loop`), nested fns (own frame, by-value capture), and calls to PHP operators other than `=`/`=&`, a short list of pure PHP builtins (`max`, `min`, `abs`, ...), keywords, or global Phel fns that are neither `^:dynamic` nor `^:redef` and whose `__invoke` has no by-reference param (reflected at compile time; undefined counts as by-ref). Everything else keeps the closure: this was a denylist once and kept leaking (`php/=`, `^:by-ref` globals, `func_get_args`). Statement position, several body forms in expression position (the flagged `let` would fall back to an IIFE and take the target and key out of the caller's frame), `recur` aimed at the fn, rest/multi-arity/named fns, tagged params and any return type, declared or inferred (the closure declares it and PHP enforces it), also keep it.
