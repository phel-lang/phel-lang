...
| `Performance/OpcacheWorkerFlags` | pure `forFileCache(loaded, dir)` → `-d opcache.enable_cli=1 -d opcache.file_cache=<dir> -d opcache.jit=disable` flag pairs (or `[]`); shared by parallel test workers (`RunFactory`) and the CLI re-exec |
