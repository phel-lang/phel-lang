...
| `Process/WorkerProcess` | the process side of that protocol: `open(command, writeFailureMessage)` (null when `proc_open` fails, so each pool phrases its own spawn error), `write()`, `tryReadFrame()`, `readStderr()`, `stdoutHandle()`, `waitForOutput()`, `isAlive()`, `terminate(signal)`. Composed by `Run\…\TestWorkerHandle` and `Mutate\…\MutantWorker`, which keep only what genuinely differs: slot assignment on one side, the in-flight deadline on the other |
