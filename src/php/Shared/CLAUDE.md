...
- `AbstractLocatedException` — base for located errors; carries `SourceLocation` + `ErrorCode` (enum, PHEL001-PHEL310: analyzer, parser, reader, lexer), plus an optional `relatedLocationNote`: a second position worth naming, already formatted, that `TextExceptionPrinter` renders under the snippet (`first defined at foo.phel:2`, #3267)
