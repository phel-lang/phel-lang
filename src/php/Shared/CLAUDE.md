...
| `Performance/OpcacheWorkerFlags` | pure `forFileCache(loaded, dir)` → `-d opcache.enable_cli=1 -d opcache.file_cache=<dir> -d opcache.jit=disable` flag pairs (JIT off: with it on OPcache writes no file cache) (or `[]`); shared by parallel test workers (`RunFactory`) and the CLI re-exec |
