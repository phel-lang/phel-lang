...
- `ExceptionPrinterInterface` — contract for exception/stack-trace rendering; implemented by `Command\Application\TextExceptionPrinter`, consumed by Run's REPL error formatter. Lives here so `CommandFacadeInterface` doesn't back-reference `Command\Domain`. Every method renders a structured exception or trace; there is deliberately no plain-string `printError()` (removed: it echoed straight to stdout, bypassing the Symfony `OutputInterface` that every command already writes through). `printStackTrace()` looks unused in PHP but is called from `src/phel/test.phel` via interop.
