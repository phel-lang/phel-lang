...
- `Exceptions/Hint/` — pure error→hint mappers (`ExceptionHintInterface` + `UndefinedSymbolHint`, `ArgumentCountHint`, `NotCallableHint`) and `ExceptionHintResolver` (first applicable hint, unwrapping one `getPrevious()` level). Shared so a host rendering Phel errors of its own reaches the same guidance the one report gives (#3264).
...
| `TagResolver` | resolves a Phel `:tag` meta value to a PHP type string: static `fromMeta()` (null-safe) + `normalizeScalar()` (Symbol→name, else non-empty string or null; expands the `TYPE_ALIASES` value-type names and `:use`-imported bare names to rooted classes, roots dotted names). **Not stateless**: holds one process-wide `private static ?Closure $useAliasResolver`, the same process-wide slot pattern as the compiler's `GlobalEnvironmentRegistry`. `GlobalEnvironment`'s constructor installs it via `setUseAliasResolver()` (reads the current namespace's `:use` table, so the last-constructed environment wins); `GlobalEnvironmentManager::reset()` clears it with `null`. Tests that install one must clear it in `tearDown()`. Single source for the analyzer type-inferrers, `MethodEmitter`, and `PhpAttributeEmitterTrait` |
...
| `Performance/OpcacheFileCache` | reads the layout of an OPcache file cache dir: `entries()`, `detectSystemId(compiledFile)` (probes the `.bin` of a file the process already compiled), `foreignEntries(id)`, `sizeInBytes()`. Layout is `<file_cache>/<32-char system id><absolute source path>.bin`, so a PHAR source yields a `<id>phar:` top-level entry |
| `Performance/OpcacheFileCachePruner` | the deletions for the above: `pruneForeignSystemIds()`, `pruneOrphanedBins()`, `clearContents()` (empties without removing, since PHP aborts when `opcache.file_cache` is missing). Symlinked directories are unlinked, never descended, so a deletion stays in the tree |
...
| `Process/CpuCountDetector` | how many worker subprocesses to spawn: `PHEL_TEST_WORKERS`, else `nproc` / `sysctl` / `/proc/cpuinfo`, else 4; `detect()` caps at 8, `detectMax()` does not. Shared by `phel test --parallel` and `phel mutate --parallel` |
| `Process/GitChangedFiles` | `changedFiles(?ref, projectDir)`: absolute paths git reports as changed (uncommitted against `HEAD` plus untracked; else since the merge base with the default branch; or `git diff <ref>`), throws `GitUnavailableException` outside a repository. Shared by `phel test --changed` and `phel mutate --changed` |
...
| `Process/PhelBinaryLocator` | static `locate()` -> the `phel` entry point of the running process (`SCRIPT_FILENAME`, else `argv[0]`, else the repo-relative `bin/phel`), so a worker runs the same code and config as its parent. Shared because `RunFactory` and `MutateFactory` may not reach into each other |
