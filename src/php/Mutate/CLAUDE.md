...
- `Application/PcovScopeWidener`: the `-d pcov.directory=/` the worker argv carries when pcov is loaded; `[]` otherwise.
...
- **Coverage-guided when a driver is there.** Every worker takes the baseline under per-test coverage attribution (`RunFacadeInterface::beginPerTestCoverage()`, xdebug in coverage mode or pcov), so each mutant runs only the tests whose coverage reaches its definition, and a definition no test reaches is `not-covered` without running anything. Without a driver every mutant runs the whole suite and the report says so. Attribution is per worker process because the collector hooks that process's `phel.test`; the baselines run in parallel so start-up costs one baseline of wall time, not N. Workers spawn with the `-d` from `Application/PcovScopeWidener`, without which pcov instruments nothing outside `<cwd>/src` and every mutant reads as `not-covered` (#3270).
- **MSI vs covered MSI.** MSI = detected / (detected + survived + not-covered): what nothing tests counts against the suite. Covered MSI = detected / (detected + survived): the quality of the tests that exist. `--min-msi` and `--min-covered-msi` gate each; `.github/workflows/mutation-phel.yml` runs `src/phel/core` weekly against both floors. A run where every mutant is `not-covered` scores covered MSI 0, not the 100 that 0/0 would give, so no floor is cleared by a run that tested nothing; the text report warns when a driver reached no mutant at all.
