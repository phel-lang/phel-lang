...
- **BigInteger** — pure-PHP arbitrary-precision signed integer (`final readonly`); base-10^9 magnitude with sign; `fromInt`, `fromString`, `add/subtract/multiply/divide/mod/gcd/pow/negate/abs`, `compareTo/equals`, `hash`, `toInt`/`fitsInPhpInt`. Implements `EqualsInterface` and `HashableInterface`. No I/O, no static state.
