...
- **Rational** : exact rational `n/d` (`final readonly`, implements `TypeInterface`); always normalised (denominator > 0, `gcd(|num|, denom) = 1`). `Rational::create($num, $den)` auto-collapses to `int` or `BigInt` when the result is integral, so `create(4, 2)` returns `int 2`, not a `Rational`. Arithmetic accepts `Rational | BigInt | int` and stays `Rational` only while the result is non-integral.
...
- **NumericOperations** : runtime dispatch helper (static-only, `final`) for `+ - * /`, comparisons, and predicates across native PHP numbers, `BigInt`, and `Rational`. Phel's arithmetic core fns route through this class because PHP's native operators do not dispatch on objects.
