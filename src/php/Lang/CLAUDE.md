...
- **BigDecimal** — arbitrary-precision signed decimal (`final readonly`, implements `TypeInterface`); stored as `BigInteger mantissa * 10^-scale`. Constructors: `fromString` (accepts optional sign, integer part, fraction, `eE` exponent, underscores), `fromInt`, `fromBigInteger`, `fromFloat` (shortest round-trip; rejects NaN/Inf). Arithmetic: `add/subtract/multiply/divideExact/negate/abs/compareTo/isZero`. Equality is value-based via `compareTo` (so `1.20M` equals `1.2M`); `hash` uses the canonical decimal form. `divideExact` extends scale up to 100 digits before throwing on a non-terminating expansion. `__toString` renders the canonical form with an `M` suffix; `toPlainString` renders without the suffix and without trimming trailing zeros. `M`-suffix literals (`1.5M`, `1.5e3M`) and the `phel.core/bigdec` constructor produce `BigDecimal` instances; `LiteralEmitter` emits them as `BigDecimal::fromString(...)` calls.
...
- **MapEntry** (`Collections/Map/MapEntry`) — typed two-element entry (`final readonly`, implements `TypeInterface, Countable, IteratorAggregate`); equal by value to a 2-element `PersistentVectorInterface`. `MapEntry::create($k, $v)` constructs; `key()`/`value()` accessors. Backs the `phel.core/map-entry` constructor and is recognised by `map-entry?`, `key`, `val`. Map iteration still yields plain 2-vectors today; `MapEntry` is opt-in.
