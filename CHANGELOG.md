...
- `(ns ... (:require ...))` now loads its namespaces under `phel eval` and `phel nrepl`, not only in the interactive REPL. The emitted require code resolved the project source dirs only when `*repl-mode*` was set, so eval/nREPL sessions walked an empty dir list, silently loaded nothing, and the analyzer then failed with `Cannot resolve symbol` on the first referred symbol. The runtime loading is now gated on build mode instead: `phel build` still keeps dependency loading out of compilation, and every other eval context (repl, eval, nREPL, run/test) resolves and loads requires
