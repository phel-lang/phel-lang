...
- Add `phel balance [paths]... [--fix]`: reports unbalanced `()`, `[]` and `{}`, and with `--fix` appends the missing closers. Scans the lexer's token stream, so `\(`, a `(` inside a string, a `;` comment or a `#"regex"` is never counted. Detection is the default, and it refuses anything with more than one plausible fix: a surplus or mismatched closer, an unterminated string, a file that will not lex, a trailing reader prefix or lone `\`, or a missing closer followed by a new top-level form, which is read from a column-0 opener, a reader prefix that started at column 0, or a definition head at any indent (#2827)
...
- LSP rename no longer corrupts source. `SymbolExtractor` wrote raw 0-based `SourceLocation` columns into `Location` and `Definition`, whose contract is 1-based because `PositionConverter` subtracts one on the way out, so every range landed one column left: renaming `add` in `(defn add [x] x)` replaced `' ad'` and produced `(defnsubd [x] x)`. The definition site was also edited twice. Go-to-definition on a `def`, document and workspace symbols, and find-references were shifted by the same column, and so are the `col`/`endCol` of `phel index --output` and the `api-daemon` JSON-RPC responses, which serialize without the LSP conversion (#3157)
...
- Type a `let` or `loop` binding, and infer a function's return type, from a collection literal or a call to `vector`, `list`, `hash-map` or `array-map`. The collection specialisations already lowered `nth`, `count`, `first` and `get` on a typed binding to a direct method call, but nothing produced such a type for ordinary code, so they never fired outside hand-annotated sources. `(let [v [1 2 3 4]] (+ (nth v 0) (nth v 2) (count v)))` 4.07x, the map equivalent 2.40x, and 3.33x when the collection comes back from a call instead. The win is in user code; the stdlib rarely binds a literal and then indexes it (#3042)
- Apply every single-use lowering to any target the emitter can type, not only a bare variable, on the rule that each emits its target once: `get`, `empty?`, `assoc`/`conj`/`dissoc`, the vector and seq accessors (`count`, `nth`, `first`, `rest`, `next`), `reduce`, `get-in`, the named accessors (`name`/`namespace`), the numeric type predicates and the `php/array` and string gates. `get-in` 2.79x, `(nth (mkvec) 2)` 2.42x, `reduce` over a vector 1.93x, `(get (mkmap) :a)` 1.78x, `(empty? (mkvec))` 1.47x, `(assoc (mkmap) :c 3)` 1.41x. Only `last`, `second` and `contains?` still require a bare variable: they emit their target three, two and two times, and a call emitted twice would run twice (#3042)
- Carry an inferred tag further: through a `let` rebinding, into a nested `let` (which `if-let`, `when-let` and `binding` all expand to), from an `if`/`do`/nested-`let` init, and from a class or `array` return type rather than only a scalar one (#2985 #2987 #3021 #3040)
- Carry a definition's declared return tag to its call sites, so a call to a `^string` function counts as a string operand and `(str "<" (escape-html s) ">")` lowers to native PHP concatenation. Previously only tagged locals and native PHP calls were typed, leaving every user and stdlib function opaque. 1.62x on a three-part concatenation, and it removes 26 runtime `str` calls from the compiled standard library. A function whose arities declare different return tags is left alone. Note that a `^int` return now takes the same native arithmetic path a `^int` local already did, which overflows to float rather than promoting to `BigInt` (#2712)
- Widen the table of `php/*` built-ins whose return type the compiler trusts from 76 entries to 125, adding `htmlspecialchars`, `mb_substr`, `strtr`, `preg_quote`, `number_format`, `md5`, `date` and more on the string side, `str_contains`/`str_starts_with`/`str_ends_with`, `is_a` and `class_exists` on the bool side, and `time`, `strcmp` and `substr_count` on the int side. `(str "[" (php/mb_substr s 0 4) "]")` 1.83x; `is_a` gives every `defstruct` predicate a `bool` return type (#2712)
- Skip the `Truthy` adapter in an `if` whose test is known to be a PHP bool, and lower `(not x)` over one to a native `!` (3.7x). This covers the comparison operators, any user `defn ^bool`, the fifteen core predicates that returned an untagged bool, and the string interop wrappers. `BooleanExprDetector` now reads its list from `PhpFunctionReturnTypes` rather than a copy that had drifted, which adds nineteen more built-ins. 1 to 7% per test, plus the return type it propagates (#2712 #2973 #3021)
- Compile a literal `(list ...)`, `(vector ...)`, `(queue ...)`, `(hash-map ...)` or `(array-map ...)` straight to its `Phel` factory: 2.2 to 3.6 times faster (#3014)

Runtime core:

- Replace rest-argument dispatch with real arities across `phel.core`. A `defn` that took `[x & args]` and then recovered its arguments with `(case (count args) ...)` allocated the rest argument, counted it, and unpacked it with `first` to get back what the caller passed positionally. Now done for the arithmetic and comparison operators, `str`, `get`, `assoc`, `reduce`, `into`, the five collection constructors, `hash-set`, `map`, `filter` and twelve more seq functions, `slice`, `dissoc`, `concat`, `swap!`, `swap-vals!`, `range`, `comp`, `repeat`, `repeatedly`, `==`, `merge-with`, `deep-merge`, `aget`, `get-in`, `max`, `min`, `inc`, `dec` and the twelve nil-rejecting arithmetic guards. Between 1.03x and 25x depending on how much work sits behind the call: `round` and `floor` 25x, `str` at four and five arguments 3.9x and 4.1x, the transducer arities and small constructors gain most, and `mapcat`, where realization dominates, gains almost nothing (#2941 #2962 #2964 #2974 #2975 #2978 #2979 #2981 #2989 #2991 #3010 #3017 #3018 #3021 #3065 #3067 #3069)
- Give every closure-returning combinator real arities: `partial` 30x, `fnil` 50x, `constantly` 9.5x, `comp` 8x, `juxt` 4.3 to 5.8x, `complement` 4.4x, `some-fn` 2.3x, `every-pred` 2.2x, plus the reducing function every transducer wraps, `completing` and `transduce` (`transduce` with `map` 4.8x, with `filter` 3.8x) (#2973 #3021 #3026 #3083)
...
- Sort natively instead of calling back into Phel per comparison, when the keys are all native ints or all native strings and no comparator other than `compare` was given: `sort` on strings 22.7x, `sort-by` on strings 13.7x, `sort-by` on ints 8.4x, `(sort-by count strings)` 5.2x. A position column keeps ties in input order. `SORT_REGULAR` is used and not `SORT_STRING`, because `compare` settles two strings with PHP's `<=>`, which orders numeric strings by value (#2973 #3003 #3004 #3079)
- Compare two native ints without the numeric-tower dispatch in `<`, `<=`, `>`, `>=`, `min` and `max` (2.5 to 3.0x), answer `zero?`, `pos?` and `neg?` with a direct PHP comparison (`pos?` and `neg?` 4.2x), and `even?` and `odd?` with one PHP modulo instead of reaching through `%` and `rem` (5.9x) (#2973 #2984)
- Add a PHP array of ints in `sum` with `array_sum` instead of spreading the collection into the variadic arm of `+`: 27.5x over 200 ints, 17.5x over 20, carrying `mean` 13.9x with it. `sum` still promotes an overflowing int total to BigInt rather than letting it become a float, by redoing it through `+`. `mean` divides with `php/fdiv` and so has never accepted a BigInt total, before this change or after (#3085)
- Seed `union`'s result from its first argument instead of re-adding every element of it to a fresh set: `(union big small)` over 200 and 5 elements 63x, `(union s)` 51x, two 20-element sets 3.0x. Non-set iterables are still added element by element (#3077)
- Build collections by collecting into a PHP array and constructing once, rather than appending through a transient or a persistent `conj` per element: `vec` 3.1x over 32 elements and 3.5x over 500, `into` a vector 3.5x, `php->phel` on an indexed array 1.97x, `kvs` 1.65x, and a two-map `merge` through the collection's own bulk merge 2.6x. Metadata is copied explicitly where the transient used to carry it (#2973)
- Write transient accumulators with `.put`/`.add`/`.find`/`.contains` rather than `get`, `assoc!` and `assoc`, which dispatch on the target's type before reaching the same methods once per element: `invert` 2.04x, `group-by` 1.50x, `select-keys` 1.26x, `set` 1.19x, `zipmap` 1.15x, `frequencies` 1.11x. `invert` and `map-invert` are two names for the same operation and had drifted apart. The `distinct` transducer tracks what it has seen in a transient set rather than a volatile holding a persistent one: 1.6x (#2973)
- Walk with `foreach` rather than `for ... :pairs`, whose destructuring was most of what these cost: `stringify-keys` 2.45x, `keywordize-keys` 2.4x, `postwalk` 2.29x, `merge-with` 2.16x, `map-invert` 1.99x, `update-vals` and `update-keys` 1.9x, `rename-keys` 1.76x (#2973)
- Step a generator-backed lazy sequence with one `Cons` per element instead of two `LazySeq` wrappers, each of which cost two sequence objects, two closures and an extra thunk call: `concat` realized 1.62x, `map` realized 1.55x. Realize 4 elements when a lazy sequence is constructed instead of the full batch of 32, later chunks still 32: constructing a `partition-by` 4.8x, a `dedupe` 3.5x, at the cost of 6-7% for a sequence consumed in full (#2973 #3061)
- Build `reductions` and `flatten` from PHP generators instead of recursing in Phel and instead of walking an eager `tree-seq`. Each `reductions` element cost a `lazy-seq` thunk plus `seq`, `first`, `rest`, `cons` and a self-call: realized 2.11x, taken 2.32x. `tree-seq` materialised every node, branches included, into a transient vector, so `flatten` walked the whole tree at construction despite handing back a lazy sequence: constructing one goes from 38.0μs to 0.63μs, `(take 3 (flatten ...))` from 47.6μs to 7.5μs, and realizing 64 leaves from 120.5μs to 69.3μs. Both stay lazy, so an infinite source is safe (#2973)
- Reach the common branch first rather than falling through a chain of predicates, in `get` (map and vector 1.6 to 1.8x, transient map by its own branch, `get-in` 1.2x), `assoc` (1.24x), `dissoc` (1.14x), `conj`, `int`, `empty?` and `seq`, and give `assoc!` the fixed arity `conj!`, `dissoc!` and `disj!` already had (1.74x) (#2960 #2961 #2963 #2965 #2973 #3071)
- Stop `all?` and `some?` re-testing `empty?` once per element by testing the first element directly and walking a seq only for the tail: `every?` over a list 2.1x, `not-any?` 1.5x, over a vector 1.4x. `some` keeps its seq walk on purpose (#3074)
...
- Collect `for` results into a PHP array instead of an atom and a persistent `conj` per element, and thread the `:reduce` accumulator through a one-slot PHP array instead of an atom (#2997 #2999)
...
- Render an element in `phel.html` without normalizing it first, tag its string-returning helpers `^string` so the `str` calls that build an attribute lower to native concatenation, join the runtime render with `implode` over a PHP array, and hoist `escape-html`'s flag constant out of the call (4.4x). A document renders 1.43x, then 1.38x, then 1.51x faster across the three; `[:span {:class "x"} "0"]` goes from 17.7μs to 7.0μs, and `phel.html` drops from 11 runtime `str` calls to 7 (#2973 #3054 #3098)
- Build every `phel.walk` branch through a transient instead of `map` plus `into`, which realized the first chunk on construction and then walked it again, once per node of the tree: 1.67x to 1.71x (#3087)
- Swap `phel.base64/encode-url` and `decode-url`'s alphabet with one `strtr` instead of a chain of `phel.string/replace` calls: 28x and 17x. All three also take fixed arities, making `decode` 5.2x (#3021)
...
- Stop `phel.pprint` printing every node twice and allocating a printer per node: leaves are no longer measured, a subtree that fits reuses the string it was measured with, and `Printer/readable` is built once. 1.60x on a deep structure, 1.48x on a small one (#3092)
- Derive each `[:map ...]` entry's shape once in `phel.schema/validate` instead of twice through two helpers, and index the entry loop rather than walking a seq over a vector: 1.23x on a four-field map, scaling with field count (#3096)
