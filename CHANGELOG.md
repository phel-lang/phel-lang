...
- A tag naming a `:use` import is resolved in the namespace that declares it. With `(:use DateTimeImmutable :as Moment)` and `(defn ^Moment make [] ...)`, a caller in a namespace that maps `Moment` to `DateTime` inferred `DateTime` as its own return type and threw a `TypeError` at call time. The definition's `:tag` now holds `\DateTimeImmutable`. Rebuilding the environment for `phel doc` or completion also no longer leaves tag resolution reading the throwaway one. (#3319)
