...
- At optimization level 2, compile `(update m k (fn [v] ...))` with a literal one-arity `fn` to a read, the fn body and one `assoc`, with no closure and no call through `update`: about 9% faster on a 90-key map. It is direct linking, so like the call inliner it stays off at the default level and on a `:redef` `update`, where `with-redefs` must still reach the call. The body then runs in the caller's PHP frame, so it lowers only when every part of it is on an allowlist that cannot tell the frames apart: literals, local and global reads, collection literals without reader metadata, `if`/`do`/`let` (and so `and`, `or`, `when`, `cond`), nested fns, PHP operators other than assignment, a few pure PHP builtins such as `max` and `abs`, keyword lookups, and global Phel fns without a `^:by-ref` param that are neither `^:dynamic` nor `^:redef`; extra arguments are held to the same list. Anything else, a nested fn, a rest param, several arities, a named fn, a tagged param, any return type (declared or inferred), `recur` aimed at the fn, a discarded result, several body forms where the result is used as a value, `apply` and a local `update` keep the runtime call. The fn is analysed once, as a fn, so its params stay untyped and macros in it see the usual `&env`. (#3322)
