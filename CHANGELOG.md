...
- `RunFacadeInterface::beginPerTestCoverage()`, `perTestCoverageByLine()` and `endPerTestCoverage()`: per-test coverage attribution for hosts that run `phel.test` themselves. `Phel\Shared\Process\CpuCountDetector` (from `Phel\Run\Application\Test`, so `RunFacade::createCpuCountDetector()` now returns the Shared class) and `Phel\Shared\Process\GitChangedFiles` with `GitUnavailableException`, shared by `phel test` and `phel mutate` (#3209)
...
- `PhelConfig::withCacheEnvVars(['MY_MODE'])` (config key `cache-env-vars`): the environment variables that take part in the compiled-code cache key. Macro expansion runs at compile time, so `(php/getenv "MY_MODE")` inside a macro bakes the current value into the emitted PHP while the cache key sees only the source, and a later run with another value was served the first expansion. A declared variable makes that flip a cache miss; only a hash of the value is stored, never the value. One fingerprint covers the project, so changing a declared value recompiles all of it: a `cache-dir` per configuration is still the way to keep several warm. `Phel\Shared\CompiledSourceHash::of()` takes the fingerprint as a third, optional argument; declaring nothing keeps the previous key, so existing caches stay warm (#3236)
...
- `phel test --reporter=github`: GitHub Actions workflow commands, so a failing or erroring assertion becomes an inline `::error file=...,line=...,title=FAIL ns/test::...` annotation on the pull request diff (the message is the same detail block the default reporter prints), each namespace that runs a test is a collapsible `::group::`, a run narrowed by `^:focus` is a `::warning`, and the counts are appended to the job's step summary when `GITHUB_STEP_SUMMARY` is set. Added next to the default (or `--testdox`) reporter automatically when `GITHUB_ACTIONS` is `true` and no `--reporter` was given; `--parallel` keeps the annotations and writes the summary once (#3214)
- `^{:skip "reason"}` and `^:skip` on a `deftest` (listed and counted as skipped with the reason, never run), `(phel.test/skip! "reason")` inside a test body (skips at run time, keeps the assertions made before it), and `^:focus` (the run narrows to the focused tests among the selected ones and prints `Focused run: N test(s), M ignored`; a focus left in fails `phel test` when `CI` is set or with `--fail-on-focus`, serial and parallel alike). All five built-in reporters render the reason (`# SKIP` in TAP, `<skipped message>` and a `skipped` count in JUnit). Test discovery now reads metadata through the var, so a tag set with `alter-meta!` at run time is honoured (#3211)
- `phel test --changed[=<ref>]`: run only the test namespaces affected by changed files, i.e. the tests of the changed namespaces and of everything that transitively requires them. Without a value it takes the uncommitted changes (untracked files included), or the changes since the merge base with the default branch when the tree is clean; with a value, `git diff <ref>`. It says what it selected (`Changed: 3 file(s), affected: 7 test namespace(s)`), composes with `--include/--exclude/--ns/--filter/--list`, and a change nothing requires exits 0 with a notice instead of a silent full or empty run. Outside a git repository it is an error (#3210)
- `phel mutate` grows `--parallel=N|auto` (a pool of workers; all load and baseline at once, mutants are handed out as workers come free), coverage-guided selection (with xdebug in coverage mode or pcov, every worker attributes tests to lines at the baseline, so each mutant runs only the tests that reach its definition and a definition no test reaches is `not-covered` without running anything), `--changed[=<ref>]` (only the source files git reports as changed), `--min-covered-msi` next to `--min-msi`, and a weekly `Mutation (Phel core)` workflow over `src/phel/core` (#3209)
...
- `(catch Name e ...)` names the class a `(defexception Name)` in the same namespace defined. The bare name resolved to the constructor fn `defexception` also defines, and a var read in catch position was a PHP syntax error; a class spelled by its bare name is now caught by type (#3211)
...
- `--warn-deprecations` (and `PHEL_WARN_DEPRECATIONS`) reports the same deprecations on a warm compiled-code cache as on a cold one. A deprecation is found while a namespace compiles, and a cached namespace was served without recompiling, so a check run after a plain `phel test` reported nothing; the notices a compile finds are now recorded whether or not the flag is on, stored with the cache entry, and raised again on every hit. Same for the always-on `\` separator announcement. The cache index format is bumped, so the first run after upgrading recompiles once. `CompilerFacadeInterface::replayDeprecations()` and `EmitterResult::getDeprecations()` are the seams (#3222)
- `phel format` is linear in the number of elements of a collection literal. Every step of the formatter's zipper copied the sibling arrays, so a literal of n elements cost O(n^2) per rule: 16 000 numbers on one line took 28.5 s to check and a 486 KB generated sprite file 95 s. A location now holds the parent's child list and an index, so a step is O(1); the same probes take 0.29 s and about 1.5 s. Output is byte-identical (#3218)
- A failing `is` reports its own line. A macro expansion rebuilt every list it contained without its source position and then stamped the macro call site onto it, so every assertion inside a `deftest` (or any other macro) carried the enclosing macro's line: `FAIL failing (fail_test.phel:4)` for an assertion on line 7. Lists a macro was given keep the position they were read at; only forms the expansion synthesised are stamped, as documented (#3228)
- `phel test --slowest` and `--last-failed` keep what the run recorded before a test itself called `phel.test/run-tests`. Every `run-tests` reset the global timings and failed-test list, a nested call included, so on this repository `--slowest=99999` listed 70 of 3009 tests. A nested run now hands the outer run's state back when it returns; at the top level nothing changes, `get-failed-tests` still reads the run that just finished (#3206)
- The scan-index cache notices a configured directory that did not exist when the index was written. `perDirFingerprint` skipped a missing directory and validation only re-checked recorded ones, so a project that ran any `phel` command before creating `tests/` (or `src/`) kept getting `Cannot find any tests in :` until `.phel/cache` was deleted; the requested directories that exist now are compared against the fingerprint on every read (#3205)
- `phel test --parallel` beats the serial run. The parent evaluated every namespace before dispatching, and each worker started with a REPL-style load that walked the whole project tree from cwd, so eight of those walks racing in the kernel cost more than the tests: on this repository 8 workers took 5.8s wall and 21s of system time against 5.6s serial, and a cold cache took 63s against 49s. The parent now evaluates only the namespaces another one requires (so a cold cache is warmed once, never raced), ships each frame's load order (requires plus every bundled `phel.*`, dependencies first), and the worker evaluates each file once and never walks dependencies. Without the sleeping `phel-test.ai` namespace, 4 workers run the suite in 1.2s against 2.7s serial and 2.3s before; a cold `--parallel=auto` takes 17s. A file that does not compile is reported once by the worker that hit it instead of being retried on two fresh workers (#3203)
