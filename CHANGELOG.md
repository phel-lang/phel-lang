...
- `re-find` stops converting the whole match array to read one string out of it, so a group-less pattern costs about three times a raw `php/preg_match` instead of fifty times it (200k calls: 612 ms to 39 ms), and a pattern with groups builds its vector from the match array directly, 23% faster. The core library now asks `re-find`/`re-matches` where it used to compare `php/preg_match`'s integer against `1`, which is what made `slurp`'s guard fail silently; `parse-long` came out 3x faster than the raw spelling it replaced (#2941, #2943)
