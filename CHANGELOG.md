...
- LSP rename no longer corrupts source. `SymbolExtractor` wrote raw 0-based `SourceLocation` columns into `Location` and `Definition`, whose contract is 1-based because `PositionConverter` subtracts one on the way out, so every range landed one column left: renaming `add` in `(defn add [x] x)` replaced `' ad'` and produced `(defnsubd [x] x)`. The definition site was also edited twice, once as a definition and once as its own reference, and the LSP spec leaves overlapping edits undefined. Go-to-definition on a `def`, document and workspace symbols, and find-references were shifted by the same one column. The `col` and `endCol` of `phel index --output` and of the `api-daemon` JSON-RPC responses shift with them: those serialize `Location`/`Definition` directly, without the LSP conversion, so a tool reading them has to drop its own compensation (#3157)
- `Phel\Lang\Collections\Exceptions\IndexOutOfBoundsException` extends SPL's `OutOfBoundsException`. `phel.core/nth` raises `\OutOfBoundsException` from its own body, but a `nth` on a statically typed collection lowers to `->get()`, which raises the former; the two shared no ancestor but `Exception`, so `(catch \OutOfBoundsException e ...)` caught the call or not depending on whether the binding happened to carry a tag. Additive: anything catching the old class still catches it (#3042)
- Three architecture tests located Gacela pillars with a glob rooted at the module directory, while Gacela 2.0 resolves them by filename suffix at any depth. A pillar one directory deeper was resolved by the framework and unchecked by `SatelliteFactoryFacadeInjectionTest`, `ExplicitServiceMapTest` and `PublicApiSurfaceTest`; all three now walk recursively, and the concrete-facade scanner also sees a nullable or union return type (#3062)
...
- Apply the vector and seq accessor lowerings (`count`, `nth`, `first`, `rest`, `next`) to any target the emitter can type, not only a bare variable, so `(count (mkvec))` specialises the way `(let [v (mkvec)] (count v))` already did. `(nth (mkvec) 2)` is 2.42x. `last` and `second` keep the variable-only restriction: they emit their target three and two times respectively, and a call emitted twice would run twice (#3042)
- Build `reductions` from a PHP generator (`Seq::reductions`) instead of recursing in Phel. Each element cost a `lazy-seq` thunk plus `seq`, `first`, `rest`, `cons` and a self-call; the generator yields the accumulator directly. `bench_reductions_realized` 2.11x, `bench_reductions_taken` 2.32x. Still lazy and still unrealized until accessed, so an infinite source stays safe (#2973)
- Infer a collection return type for a function whose tail is a collection literal or a call to `vector`, `list`, `hash-map` or `array-map`, so a caller's binding picks it up and the collection specialisations fire there too. `(let [v (mkvec)] (+ (nth v 0) (nth v 2) (count v)))` is 3.33x, the map equivalent 2.22x. This is the half of #3042 that `let` bindings alone could not reach. The interface names are emitted rooted: a generated file declares its own namespace, and an unrooted `Phel\Lang\...` in a return type resolves against it and raises a `TypeError` (#3042)
- Type a `let` or `loop` binding whose init is a collection literal or a call to `vector`, `list`, `hash-map` or `array-map`. The collection specialisations already lower `nth`, `count`, `first` and `get` on a typed binding to a direct method call; nothing produced such a type for ordinary code, so they never fired outside hand-annotated sources. `(let [v [1 2 3 4]] (+ (nth v 0) (nth v 2) (count v)))` is 4.07x, `(let [m {:a 1, :b 2}] (+ (get m :a) (get m :b)))` 2.40x. The stdlib itself gains little (23 of 409 generic `first` dispatches), since it rarely binds a literal and then indexes it; the win is in user code (#3042)
- Step a generator-backed lazy sequence with one `Cons` per element instead of two `LazySeq` wrappers. `LazySeq::fromGenerator`'s thunk built a `LazySeq` and called `cons` on it, and `cons` wraps its own result in a further `LazySeq` holding a further thunk, so every element cost two sequence objects, two closures and an extra thunk call. Walking is where an unchunked lazy sequence spends its time, so this reaches everything generator-backed: `bench_concat_realized` 1.62x, `bench_map_realized` 1.55x. Construction, and the chunked sequences (`interleave`, `dedupe`), are unchanged (#2973)
- Write the transient accumulator in `select-keys`, `invert` and `set` with `.put`/`.add` and read the source with `.contains`/`.find`, as `frequencies` and `group-by` already do. `bench_invert` 2.04x, `bench_select_keys` 1.26x, `bench_set` 1.19x. `invert` and `map-invert` are two names for the same operation and had drifted apart, one rewritten and one not; `bench_invert` now sits at 75.5μs against `map-invert`'s 74.7μs (#2973)
- Write the transient accumulator in `frequencies` and `group-by` with `.find` and `.put` rather than `get` and `assoc!`/`assoc`, and walk with `foreach` rather than `for ... :reduce` / `for ... :pairs`. Both dispatch on the target's type before reaching the same methods, once per element. `bench_group_by` 1.50x, `bench_frequencies` 1.11x; over 32 elements in eight buckets `group-by` goes from 87.3μs to 55.0μs (#2973)
- `BooleanExprDetector` reads the bool-returning built-ins from `PhpFunctionReturnTypes` instead of keeping its own copy. The copy had already drifted: `ctype_alpha`, `ctype_digit`, `ctype_alnum` and `ctype_space` were in the table but not in it, so an `if` over one paid the truthy adapter for no reason. Nineteen built-ins are newly recognised, worth 1.03x to 1.08x on an `if` over one of them (#2712)
- Widen the table of `php/*` built-ins whose return type the compiler trusts, from 76 entries to 125: `htmlspecialchars` and its siblings, `mb_substr`, `strtr`, `preg_quote`, `number_format`, `get_debug_type`, `md5`, `date`, `serialize` and more on the string side, `str_contains`/`str_starts_with`/`str_ends_with`, `is_a`, `class_exists` and the remaining `ctype_*` on the bool side, `time`, `strcmp` and `substr_count` on the int side. A concatenation around one of the string entries lowers to native PHP `.`: `(str "<" (php/htmlspecialchars s) ">")` is 1.79x, `(str "[" (php/mb_substr s 0 4) "]")` 1.83x. `is_a` gives every `defstruct` predicate a `bool` return type. The bool and int entries type the generated signature but do not move a measured `if` (#2712)
- Tag the string-returning helpers in `phel.html` with `^string`, so the `str` calls that build an attribute lower to native PHP concatenation. A document renders 1.38x faster, one of void elements 1.27x, and a single `[:span {:class "x"} "0"]` goes from 10.2μs to 7.0μs; `phel.html` drops from 11 runtime `str` calls to 7. `render-html` is tagged too, which its raw-string fix above makes sound (#2973)
- Carry a definition's declared return tag to its call sites. A call to a `^string` function now counts as a string operand, so `(str "<" (escape-html s) ">")` lowers to native PHP concatenation instead of a runtime `str`, and a `^int` function does the same for arithmetic. Previously only tagged locals and native PHP calls were typed, which left every user and stdlib function opaque. Worth 1.62x on a three-part concatenation and 1.68x on a five-part one, and it removes 26 runtime `str` calls from the compiled standard library. A function whose arities declare different return tags is left alone. Note that a `^int` return now takes the same native arithmetic path a `^int` local already did, which overflows to float rather than promoting to `BigInt` (#2712)
...
- Give the closures returned by `complement`, `constantly`, `some-fn` and `every-pred` real arities, as `partial`, `fnil` and `comp` already had: 4.4x, 9.5x, 2.3x and 2.2x to call (#2973)
- Give the reducing function every transducer wraps real arities, and the same for `completing` and `transduce`: `transduce` with `map` 4.8x, with `filter` 3.8x (#2973)
- Realize 4 elements when a lazy sequence is constructed instead of the full batch of 32; later chunks are still 32. Constructing a `partition-by` 4.8x, a `dedupe` 3.5x; a sequence consumed in full pays one extra chunk boundary, 6-7% at 32 elements (#3061)
