...
- `assoc` of the value an index already holds returns the vector itself instead of copying the path: 2.4x faster on a 100-element vector, 1.4x through `phel.core/assoc`. A struct does the same. A write of a new value costs about 3% more on a vector and about 5% less on a hash map. Maps already returned themselves for an unchanged value. `Phel\Lang\Collections\ValueIdentity` is new public PHP API. (#3321)
