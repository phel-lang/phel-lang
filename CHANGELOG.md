...
- Compile `(update m k (fn [v] ...))` with a literal one-arity `fn` to a read, the fn body and one `assoc`, with no closure and no call through `update`: about 10% faster on a 90-key map. Extra arguments, destructured params and nested fns capturing a param are covered. A rest param, several arities, a named or tagged fn, a `{:pre :post}` map, `recur` in the body, a body that writes to a local of the enclosing scope, a discarded result, `apply` and a local `update` keep the runtime call. (#3322)
