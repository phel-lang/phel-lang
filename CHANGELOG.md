...
- 3-arg `(deref future timeout-ms timeout-val)` form for time-bounded blocking on a `PhelFuture`; returns `timeout-val` if the future doesn't complete within the given milliseconds, matching Clojure's `deref` timeout semantics. The 3-arg form is only supported on `PhelFuture` — passing any other deref-able raises `InvalidArgumentException` (#1313)
- `future-cancel`, `future-cancelled?`, and `future-done?` functions for working with `PhelFuture` lifecycle. `future-cancel` signals the future's internal AMPHP `DeferredCancellation` token, causing pending and subsequent `deref` calls to raise `CancelledException` (1-arg form) or return the timeout fallback (3-arg form). Due to AMPHP's cooperative fibers, the future's body keeps running until its next cancellation checkpoint — this differs slightly from Clojure's JVM-thread interrupt semantics, and is documented in the function docstrings. `future-cancelled?` returns `true` iff `future-cancel` was called; `future-done?` is an alias for `realized?` when applied to a `PhelFuture` (true for completed, failed, or cancelled futures) (#1313)

#### Reader & Compiler
- Clojure-style character literals `\a`, `\A`, `\1`, `\space`, `\newline`, `\tab`, `\formfeed`, `\backspace`, `\return`, `\uNNNN` (Unicode), `\oNNN` (octal), and punctuation literals `\(`, `\)`, `\[`, `\]`, `\{`, `\}`, `\\`, `\"`, `\'`, `\,`, `\;`. Since PHP has no distinct character type, char literals compile to single-character PHP strings (UTF-8 encoded for Unicode escapes). FQN parsing (`\Phel\Lang\Symbol`) is preserved — disambiguation is by lookahead: `\<single-char>` followed by a delimiter is a char literal, `\Word\Word` stays an FQN (#1283)
- Clojure-style var-quote reader syntax `#'foo`. Phel has no first-class Var type, so `#'foo` is accepted and read as the bare symbol reference `foo` — `(#'bar)` evaluates identically to `(bar)`. Previously the reader treated `#'bar` as the start of a bare `#` line comment, breaking common idioms like `(var? #'bar)` with "Unterminated list (EOF)". Unblocks `.cljc` test suites (e.g. clojure-test-suite `var_qmark.cljc`) that use `#'` with `defmulti`/`defprotocol` (#1317)
...
- `fn` accepts an optional name symbol as the first argument (e.g. `(fn my-name [x] x)` and `(fn my-name ([x] ...) ([x y] ...))`), matching Clojure's `(fn name? ...)` signature for `.cljc` interop. The name is bound inside the body, so named fns can self-recurse without `recur` — `(fn fact [n] (if (zero? n) 1 (* n (fact (dec n)))))` works directly. Multi-arity named fns (`(fn foo ([x] (foo x x)) ([x y] [x y]))`) resolve the name across arities. Anonymous `(fn [x] x)` is unchanged (#1279, #1299)
...
- Clojure-style `N`-suffixed integer literals (`1N`, `-123N`, `1_000_000N`) and `M`-suffixed decimal literals (`1M`, `1.5M`, `-1.1M`, `1.5e3M`). Phel has no first-class BigInt/BigDecimal types, so the suffix is stripped and the value is parsed as a plain PHP int/float — callers that need values beyond `PHP_INT_MAX` or arbitrary-precision decimals must use an explicit library. The literals are accepted purely for `.cljc` source compatibility (#1325)
...
- `coll?` predicate returning `true` for persistent collections — vectors, lists, hash-maps (including sorted-map), structs, sets (including sorted-set), and lazy-seqs — and `false` for everything else (strings, numbers, `nil`, booleans, keywords, symbols, plain PHP arrays). Matches Clojure's `IPersistentCollection`-based definition; plain PHP arrays are excluded deliberately since they are a language primitive rather than a persistent collection (`indexed?`/`associative?` remain the right predicates for that) (#1336)
- `conj!` transient-collection mutator with Clojure-compatible arities: `(conj!)` returns a new empty transient vector, `(conj! tcoll)` returns `tcoll` unchanged, `(conj! tcoll v)` mutates `tcoll` in place by appending/adding/putting depending on the concrete transient type, and the variadic `(conj! tcoll v & more)` reduces over the remaining values. Raises `InvalidArgumentException` when the first argument is not a transient vector, hash-set, or hash-map — persistent collections, `nil`, and scalars are rejected, matching Clojure's `ITransientCollection`-based contract (#1338)
- `some-fn` higher-order predicate combinator: `(some-fn p1 p2 ...)` returns a function that, when invoked with any number of arguments, returns the first logical-true value produced by applying any of the composing predicates to any of its arguments (and `nil` otherwise). Short-circuits on the first truthy result — predicates after it are not tried and arguments after it are not inspected. Each predicate is consulted against all arguments in order before advancing to the next predicate, matching Clojure's `some-fn` semantics (#1339)
- `counted?` predicate returning `true` when a collection can report its length in constant time — persistent vectors, lists, hash-maps (including sorted-map), structs, and sets (including sorted-set) — and `false` otherwise. Lazy sequences (including `(range)` and `(range 3)`) are deliberately excluded since counting them requires realizing the entire sequence; strings, numbers, `nil`, plain PHP arrays, and all other non-persistent types are also rejected. Matches Clojure's `counted?` semantics, which mirror the `clojure.lang.Counted` marker interface (#1340)
- Single-arity `(drop-last coll)` form, equivalent to `(drop-last 1 coll)`, matching Clojure's `drop-last` signature. The existing two-arity `(drop-last n coll)` form keeps working unchanged. Unblocks `.cljc` test suites such as clojure-test-suite's `drop_last.cljc` (#1343)
...
- `=` on lazy sequences no longer eagerly realizes an infinite side. `LazySeq::equals`, `ChunkedSeq::equals`, and `AbstractPersistentVector::equals` now short-circuit on object identity and otherwise walk both operands pairwise via lockstep iterators, so `(= r (deref (atom r)))`, `(= [] (range))`, `(= [1 2 3] (range))`, and `(= (range) [1 2 3])` all return immediately instead of crashing with a PHP integer-overflow allocation error. Comparing two infinite sequences still loops forever, matching Clojure semantics (#1286)
