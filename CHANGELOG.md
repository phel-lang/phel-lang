...
- PHP interop completion/hover/signature help now resolves the receiver type through multi-hop `php/->` chains (walking each method's reflected return type), factory bindings (`[x (php/:: \Foo make)]`), and indirect `let` rebinds, wherever the return type is reflectable (follow-up to #2431; #2464)
