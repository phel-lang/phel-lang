...
- `phel doc <search>` now says `No function matches "<search>"` and points at a shorter search term, instead of rendering an empty header-only table that read as a broken command. The exit code stays 0, because a search finding nothing is an answer rather than an error, and `--format=json` still answers `[]`
- `phel lint`'s `phel/shadowed-binding` and `phel/unused-binding` now understand `for`/`dofor`/`foreach` heads. Those heads are `binding :verb coll-expr` triples plus `:let`/`:when`/`:while`/`:reduce` clauses, or a `[key value coll]` tuple, not `let`-style pairs, so every collection expression was being read as a bound name: `(for [[k v] :pairs form] ...)` reported `form` as both shadowed and unused. `foreach` was in neither rule's form list at all, so a shadowing or unused `foreach` binding went unreported
- Editor completion no longer offers the collection expression of a `for`/`dofor` head as a local binding, and now offers the bindings of a `foreach` head, which it never saw
...
- `phel lint`'s `phel/discouraged-var` now keys off real `:deprecated` metadata instead of guessing from the docstring. A docstring that merely mentions the word (documenting a `:deprecated` map key, or a deprecated PHP builtin) no longer marks the definition discouraged, and the deprecated definition's own declaration is no longer reported as a use of itself
- `phel watch` no longer selects the `inotify` backend when `ext-inotify` is loaded but `inotifywait` is not on PATH. The watcher only ever shells out to the binary, so it now probes for that alone and falls back to polling
- `--warn-deprecations` no longer blames your file for a backslash namespace separator written inside a `phel.core` macro: calling `(delay ...)` reported `\Phel\Lang\Delay` at your own source line, a deprecation you never wrote and could not remove. Expanded forms now carry the definition they came from, so a stdlib macro stays silent while a macro in your own project is reported against the file that defines it, and a `\` you typed yourself still warns exactly as before
- `--warn-deprecations` no longer blames your file for a `:deprecated` definition called inside someone else's macro: expanding a macro that calls a deprecated `def`/`defn` reported it at your call site, which you cannot edit. The notice now names the file that defines the macro and adds the call site it was reached through, a bundled `phel.core` macro stays silent, and a deprecated definition you call yourself still warns exactly as before. `^:reference` notices raised from a macro expansion follow the same rule
- A namespace with kebab-case segments (`my-app.cross-require.lib`) is no longer re-evaluated on every `(:require ...)` in the REPL, nREPL and `phel eval`: the already-loaded check compared the canonical name against the munged registry key, so kebab-case namespaces always looked unloaded and their top level (and any state it set up) ran again. Dependency resolution also accepts the legacy backslash separator in a seed or a declared dependency, instead of silently resolving to nothing
...
- `phel lint <dir>` no longer prints "No lint issues found" and exits 0 when the directory cannot be read. It exits 2 with `Cannot read directory to lint: <path>`, matching what an unreadable file already did
- `phel ns <namespace>` now exits 1 with `Namespace "..." not found in any source directory.` for a name that resolves to nothing, instead of printing a plausible-looking `phel.core`-only listing and exiting 0. `phel run` already refused the same input
- A rejected atom validator now names the value: `Atom validator rejected the value: -1`. The rendering is bounded, so a collection or a lazy seq shows as `#object[<type>]` and a long string is truncated, never realized or dumped in full
- A circular namespace dependency now reports the whole cycle, `Circular dependency detected: a -> b -> a`, instead of naming only the namespace the walk re-entered
- `--parallel 0` now reports `--parallel must be >= 1, got 0.`, naming the rejected input like every sibling `phel test` option message already did
...
- `(empty? (eduction ...))` (and any other non-countable iterable, such as a PHP generator) now answers by pulling a single element instead of raising "object is not countable". `count` still refuses such a source on purpose: counting one means draining it, which re-runs the whole pipeline and its side effects. It now says so, and points at `(count (into [] coll))`, instead of failing with the PHP-level "Object of class Phel\Lang\Eduction could not be converted to string" that the old message produced for any non-stringable value
...
- `(first (eduction ...))` now returns the first element instead of raising `Cannot use object of type Phel\Lang\Eduction as array`, and the same goes for any other lazily consumed source such as a PHP generator. `first` needs exactly one element, so it sits with `empty?` rather than with `count`, which still refuses such a source because counting it means draining a pipeline that caches nothing. An eduction re-runs its chain, so `first` on one is repeatable
- Asking a token stream for its code snippet no longer dies with `Call to a member function getStartLocation() on null` when there is no significant token to anchor on. It happens while another error is already being reported, so the crash replaced the real diagnostic with a PHP `Error`. A trivia-only stream (whitespace, comments, `#_`) now reports the trivia itself, and a stream advanced past its end reports an empty snippet at an unknown location
- `--warn-deprecations` no longer breaks the build on a `^:reference` fn param. The `^:reference` check is the only deprecation detector that runs while the emitter is building PHP inside an output buffer, so under PHP CLI's default `display_errors` the notice text was written into that buffer and spliced into the generated code, failing the compile with `syntax error, unexpected token ":"`. Every compiler deprecation notice now has its display pinned to stderr while it is raised, so no diagnostic can corrupt program output; a `display_errors` you turned off stays off
- An unterminated list, vector or map whose token stream ends without an end-of-file token now still reports `Unterminated list starting at line N. Did you forget a closing ')'?`. The error path asked the exhausted stream for a current token to anchor the message on, which raised `Token generator exhausted unexpectedly` and destroyed the diagnostic it was in the middle of building
- `phel config` now ends with a `N configuration error(s) found.` summary when the effective config has errors, and points at `phel doctor` for the pass/fail verdict. The command still exits 0 on purpose (it is a dump that succeeded, and `--format=json | jq` pipelines gate on that), so the output is the only place the errors can be made unmistakable
- `filter`, `take-while` and `drop-while` now use Phel truthiness for the predicate result instead of PHP's. A predicate returning a logically-true-but-PHP-falsy value (`0`, `0.0`, `""`, `"0"`, `@[]`) was silently dropping elements: `(filter identity [0 1 2])` returned `@[1 2]` and `(take-while identity [1 0 2])` stopped at `@[1]`, while the transducer arities of the same functions (`(into [] (filter identity) [0 1 2])`) already returned all three. Both arities now agree, and only `nil` and `false` are treated as false

### Changed

- (PHP API) `Phel\Shared\Api\Definition` gains a `deprecated` field (the `:deprecated` reason, `''` when absent) plus `isDeprecated()`, and `toArray()` serializes it. The constructor argument is optional, so existing call sites keep working
...
- **BREAKING** (PHP API, implementers only): `Phel\Shared\Facade\ApiFacadeInterface` now declares the whole semantic-analysis surface instead of 5 of 14 methods. Added: `analyzeSource`, `indexProject`, `extractDefinitions`, `resolveSymbol`, `findReferences`, `completeAtPoint`, `phpInteropHoverAt`, `phpInteropSignatureAt`, `phelSignatureAt`. `Phel\Shared\Facade\FormatterFacadeInterface` gains `formatString`. Callers are unaffected; anything *implementing* either interface must add the new methods. This is what lets `Lint`, `Lsp` and `Watch` inject the contracts rather than the concrete `ApiFacade` / `FormatterFacade`, as `src/php/CLAUDE.md` requires. Only `Lsp -> LintFacade` still binds a concrete facade, because Lint has no contract yet
- **BREAKING** (PHP API): the five semantic-analysis transfers named by `ApiFacadeInterface` moved from `Phel\Api\Transfer\` to `Phel\Shared\Api\`, joining `PhelFunction` and `CompletionResultTransfer` which already lived there: `Diagnostic`, `ProjectIndex`, `Definition`, `Location`, `Completion`. A leaf contract cannot name a `Phel\Api` class without making `Api <-> Shared` cyclic, so the payload types belong in `Shared`. Class shapes are unchanged, imports are not. `Phel\Api\Transfer\` keeps the PHP-interop reflection types (`PhpInteropCall`, `PhpInteropClass`, `PhpInteropContext`, `PhpInteropSignature`), which never cross the facade
...
- **BREAKING** (PHP API): removed `Phel\Shared\Exceptions\ExceptionPrinterInterface::printException(AbstractLocatedException $e, CodeSnippet $codeSnippet): void` and its `TextExceptionPrinter` implementation, the last side-effecting sibling of `printError()`. Nothing called it: it only wrapped `getExceptionString()` in an error-log write that `CommandExceptionWriter` already performs, while every command renders located exceptions through `getExceptionString()` and its own `OutputInterface`. Custom implementations of the interface can drop the method
