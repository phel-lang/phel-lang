...
- **BREAKING** (installation): `gacela-project/gacela` moves to **2.0**, which drops `AbstractDependencyProvider`, `GacelaConfig::addMappingInterface()`, `DocBlockResolverAwareTrait` and the internal dual provider-resolution path, and raises `gacela-project/container` to `^1.0` (where `Gacela\Container\Container` is `final` and composed rather than extended). Phel needed no source change for any of it: the removed APIs were already unused, every class resolving a pillar through `ServiceResolverAwareTrait` already declares it with `#[ServiceMap]`, and nothing type-hints the upstream concrete container. A project embedding Phel that still uses those APIs must migrate on gacela 1.21 first, which emits a runtime notice for each
