...
- At optimization level 2, compile `(update m k (fn [v] ...))` with a literal one-arity `fn` to a read, the fn body and one `assoc`, with no closure and no call through `update`: about 10% faster on a 90-key map. It is direct linking, so like the call inliner it stays off at the default level and on a `:redef` `update`, where `with-redefs` must still reach the call. Extra arguments, destructured params and nested fns capturing a param are covered. The fn is analysed once, as a fn, so its params stay untyped and macros in it see the usual `&env`; `:pre`/`:post` maps keep working. A rest param, several arities, a named fn, a tagged param or scalar return type, `recur` aimed at the fn, a body that writes to a local of the enclosing scope, a discarded result, `apply` and a local `update` keep the runtime call. (#3322)
