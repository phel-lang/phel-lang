...
- **BREAKING (PHP API)**: `Phel\Filesystem\FilesystemFacadeInterface` moves to `Phel\Shared\Facade\FilesystemFacadeInterface`, where every other cross-module facade contract already lives. `Phel\Fiber\FiberFacadeInterface` is removed: no module injected it and `phel.core` builds `Phel\Fiber\FiberFacade` directly, so type-hint the facade instead. Both are public FQCN changes, which is why they land before `1.0.0`. (#2859)
- **BREAKING (PHP API)**: `MetaInterface::withMeta()` always returns a copy and leaves the receiver unchanged. `MetaTrait` no longer mutates Symbols, Keywords, Atoms or functions in place, and every implementation is marked `#[NoDiscard]`. Keep the returned value. Mutable atom metadata now uses explicit `Atom::alterMeta()` and `Atom::resetMeta()` operations. Analyzer type inference replaces tagged Symbols while preserving typed signatures and optimized local operations. (#3311, ADR 0019)
...
- `phel bench --ab=<git-ref> --pairs=N` compares the working tree against a git ref in one sitting, interleaved: side A runs from a temporary `git worktree` of the ref, side B is the working tree, A then B once per pair, with the same `--filter`, `--revs`, `--iterations` and `--warmup`. Each row reports the mean of the per-pair deltas and how many pairs agree on the sign, and says `noise` when they do not all agree, so a machine warming up between two runs can no longer fake a regression. With `--tolerance` the run fails only when a benchmark is slower by more than the tolerance in every pair. The worktree is removed afterwards, also on failure or Ctrl-C, and the working tree is never checked out or stashed. `--ab` together with `--store` or `--ref` is an input error. See "Comparing against a git ref" in `docs/benchmarking.md`. (#3324)
...
- `assoc` of the value an index already holds returns the vector itself instead of copying the path: 2.4x faster on a 100-element vector, 1.4x through `phel.core/assoc`. A `rest` or `subvec` of a vector and a struct do the same. A write of a new value costs about 3% more on a vector and about 5% less on a hash map. Maps already returned themselves for an unchanged value. `Phel\Lang\Collections\ValueIdentity` is new public PHP API. (#3321)
- At optimization level 2, compile `(update m k (fn [v] ...))` in tail position with a literal one-arity `fn` to a read, the fn body and one `assoc`, with no closure and no call through `update`: about 9% faster on a 90-key map. It is direct linking, so like the call inliner it stays off at the default level and on a `:redef` `update`, where `with-redefs` must still reach the call. The body then runs in the caller's PHP frame, so it lowers only where that frame ends at once (return position, outside a `try`) and only when every part of it, and every extra argument, is on an allowlist that cannot tell the frames apart: literals, local and global reads, collection literals without reader metadata, `if`/`do`/`let` (and so `and`, `or`, `when`, `cond`), PHP operators other than assignment, a few pure PHP builtins such as `max` and `abs`, keyword lookups, and global Phel fns without a `^:by-ref` param that are neither `^:dynamic` nor `^:redef`. Anything else, a nested fn, a rest param, several arities, a named fn, a tagged param, any return type (declared or inferred), `recur` aimed at the fn, `apply` and a local `update` keep the runtime call. The fn is analysed once, as a fn, so its params stay untyped, macros in it see the usual `&env`, and it never types the enclosing fn's params. (#3322)
- Compile `(assoc-in m [k1 k2 ...] v)` and `(update-in m [k1 k2 ...] f & args)` with a literal path to one `\Phel\Lang\AssocIn` call instead of the `[k & ks]` recursion: on a 9-key map, 1.9x faster for two keys, 2.5x for eight, 2.3x through a vector level, and 2.8x for `update-in` with two extra arguments, which no longer go through `apply` at every level. Maps and vectors are written inline; every other level goes through `get` and `assoc`, so nil targets, missing levels, appending at a vector's count, structs, transients and the errors for a bad index or a non-associative level are unchanged. A path from a variable, `apply` and a local `assoc-in` / `update-in` keep the runtime call. `Phel\Lang\AssocIn` is new public PHP API, since compiled code calls it. (#3328)
- `into` with a transducer reduces through `(xf conj)` directly instead of going through `transduce` and `completing`, every core transducer builds one function instead of two, and `reduce` no longer builds a closure per element. On four maps, `(into [] (comp (map f) (filter :alive)) coll)` drops from 14.2μs to 11.8μs, 1.28x the cost of `(filterv :alive (mapv f coll))` instead of 1.54x. `(into [] (map f) coll)` drops from 10.5μs to 8.1μs, `(transduce xf conj [] coll)` from 14.1μs to 12.5μs, and `(reduce + 0 v)` over 32 ints from 9.6μs to 6.8μs. What is left of the gap is building the pipeline on every call: with the transducer built once and reused, `into` is within 15% of `filterv` over `mapv`. Results are unchanged, including early termination through `reduced`, stateful transducers and the completion step. One case changes, toward Clojure: a custom transducer that calls its reducing fn with extra arguments reaches `conj`'s variadic arity instead of throwing, since `into` no longer wraps `conj` in `completing`. (#3323)
...
- A tag naming a `:use` import is resolved in the namespace that declares it. With `(:use DateTimeImmutable :as Moment)` and `(defn ^Moment make [] ...)`, a caller in a namespace that maps `Moment` to `DateTime` inferred `DateTime` as its own return type and threw a `TypeError` at call time. The definition's `:tag` now holds `\DateTimeImmutable`. The members of a union `(Moment null)` or intersection `[Access Sized]` tag on `defstruct` and `definterface` resolve the same way, where they used to reach the generated PHP unqualified. Rebuilding the environment for `phel doc` or completion also no longer leaves tag resolution reading the throwaway one. (#3319)
- `assoc` on a map no longer drops a value that is `=` to the stored one but distinguishable from it. `(assoc {:t 0.0} :t -0.0)` kept `0.0`, so `(/ 1 (:t m))` gave `##Inf` instead of `##-Inf`. A `BigInt` over an equal `int` kept the `int`, and a vector carrying new metadata was replaced by the old one without it. The unchanged-value shortcut now compares by identity, as Clojure does, with a signed zero counted as a change. Applies to every map flavour and to transients. (#3321)
- `assoc` of a PHP array over an equal one always writes it. The unchanged-value shortcut compared arrays with `===`, which walks them element by element: two distinct recursive arrays threw `Nesting level too deep`, and an array holding other references to equal values kept the old array. A PHP array now always counts as a change. Applies to every map flavour, transients, structs, vectors and sub vectors. (#3321)
- At optimization level 2, an `update` with a literal `fn` keeps the closure when a `let` binding, `if` test or `truthy?` argument in the body may hold an object the body built. Spliced, that object stayed alive in the caller's frame until after `assoc`, where the closure released it before: a destructor ran later, and a custom target's `assoc` could see the object alive, through the destructor or a `WeakReference`. A Phel fn or keyword call counts, unless the fn declares a scalar return type, and so does a collection, fn or object literal. A destructured param binds from a lookup and now keeps the closure too. (#3322)
- `phel test --parallel` workers run with the OPcache JIT off. A php.ini that turned the JIT on (setup-php does, on every GitHub runner) reached every worker once Phel enabled the CLI OPcache for them, and with the JIT on OPcache writes nothing to the file cache, so the pool shared no compiled code. Workers were the only Phel processes running the JIT, and on arm64 macOS they died with SIGBUS partway through a run. (#3334)
- `phel test --parallel` no longer hangs when a worker writes more to stderr than the pipe holds. The parent read a worker's stderr only after the worker died, so a worker replaying enough deprecation notices blocked on the write and never answered. A worker that dies now reports its exit code or the signal that killed it, and what it left on stdout, where a PHP fatal error goes. Before, the report showed only stderr, which was often empty. A worker that writes to stdout outside the frame protocol no longer aborts the whole run with `Failed to decode worker frame`; it is reported as a crash and replaced. A namespace that kills its worker on all three attempts no longer aborts the whole run: the next namespace goes to a fresh worker instead of the dead one. (#3334)
...
- `Phel\Lang\DynamicScope::$boundNames`, the per-name gate that replaces the `$anyActive` fast path on the global-read path. `$anyActive` is unchanged and still latches (#3179)
...
- LSP navigation no longer lands in the build output directory. The project indexer walked the whole workspace root, and a project that has been built holds copies of its own `.phel` sources under `out/`, so jumping to a definition could open the compiled copy instead of `src/` or `vendor/`. The excluded directory is read from the project's `PhelConfig`, so a custom `destDir` is honoured; `vendor` is deliberately still indexed, since navigating into the core library is the point (#3155)
- A map or set holding a `nil` key iterated one entry short of its own `count`. The hash map keeps the `nil` key outside the trie, because `nil` has no hash path, and its iterator walked only the trie, so `keys`, `vals`, `for ... :pairs` and `into` all silently dropped that entry while `count` and `get` still reported it. Only maps large enough to use the hash representation were affected (#3172)
...
- Compile a literal `(list ...)`, `(vector ...)`, `(queue ...)`, `(hash-map ...)` or `(array-map ...)` straight to its `Phel` factory: 2.2 to 3.6x (#3014)

Runtime core:

- Gate the dynamic-scope lookup by name rather than by a single process-wide latch. `Phel::getDefinition` runs on every resolved symbol and is 10.5% of runtime self cost; once anything in a process established a dynamic binding, every global read paid the fiber-local scope check for the rest of the run, 0.153μs against 0.046μs for the registry read alone. A run that binds one var no longer slows reads of every other var: 0.164μs to 0.102μs, and the cost is now the same whether or not a binding exists (#3179)
...
- Write transient accumulators with `.put`/`.add`/`.find`/`.contains` instead of `get`, `assoc!` and `assoc`: `invert` 2.04x, `group-by` 1.50x, `select-keys` 1.26x, `into` a set 1.23x, `set` 1.19x, `zipmap` 1.15x, `into` a map 1.14x, `frequencies` 1.11x. The `distinct` transducer tracks what it has seen in a transient set: 1.6x (#2973)
...
- Walk the entries a hash-map node holds when it splits into an `ArrayNode` instead of probing all 32 slots: 1.06x (#2973)
...
- Derive each `[:map ...]` entry's shape once in `phel.schema/validate` instead of twice: 1.23x on a four-field map, scaling with field count (#3096)
