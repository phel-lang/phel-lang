...
- `phel test --parallel` workers run with the OPcache JIT off. A php.ini that enables the JIT (setup-php does on every GitHub runner) reached every worker once Phel turned the CLI OPcache on for them: the arm64 macOS runners killed workers with SIGBUS, and with the JIT on OPcache writes no file cache, so the pool shared no compiled code. A dead worker is now reported with its exit code or signal and what it left on stdout and stderr. Also fixed: a worker writing more to stderr than the pipe holds no longer hangs the run, output outside a frame is reported as a crash instead of aborting the run, and a namespace that kills its worker on every retry no longer takes the rest of the queue down with it. (#3334)
