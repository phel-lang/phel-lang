...
- `set!` now also assigns a dynamic var's current thread-local binding: `(binding [*x* 1] (set! *x* 2))`, the second shape Clojure's `set!` covers. It throws when no `binding` frame is active, so it can never change a root by accident; `alter-var-root` remains the way to do that. `docs/spec/clojure-divergences.md` now lays the three var-mutation operations side by side with Clojure, including where `set-var` sits
- `set!` sets an object or class property the way Clojure spells it: `(set! (.-total o) 10)`, the Clojure name for `php/oset`. It evaluates to the assigned value, where `php/oset` evaluates to the target object. `docs/spec/clojure-divergences.md` now records which mutating `php/*` forms got Clojure names and why `php/apush`, `php/aunset`, `php/ref` and `php/callable` keep the prefix, plus the fact that `aset` and `set!` are macros rather than functions
...
- Qualified members work in value position, the two Clojure 1.12 shapes PHP allows: `\C/m` is a static method as a value (`(map \Phel\Lang\Keyword/create ["a" "b"])`) and `\C/.m` is an instance method as a function of its receiver (`(map \App\User/.name users)`). `\C/CONST` still reads the constant, and a class carrying both a constant and a static method under one name keeps resolving to the constant. `\C/new` is deliberately not a constructor: `Foo::new()` is a legal, common PHP named constructor, so claiming the name would break real code
...
- [Language surface specification](docs/spec/language-surface.md): the normative, frozen half of the language. Reader syntax, the closed special-form list, the standard-library contract, and the stated non-goals (no STM, no agents, no `core.async`, no character type). A test parses the special-form table and compares it against the analyzer's own dispatch registry, so the spec cannot drift from the compiler
- [Clojure divergences](docs/spec/clojure-divergences.md): every place Phel deliberately behaves differently from Clojure, catalogued by cause, derived from the `:phel` branches in the nightly `clojure-test-suite` run. A behaviour listed there is a decision; anything else that differs is a bug worth reporting
- [Upgrade guide 0.49 to 1.0](docs/migration/upgrade-0.49-to-1.0.md): what to change, in the order to change it, starting with how to find out whether anything needs changing at all. Calls out `,` as the one removal that fails silently rather than failing to parse, and that the sweep has to cover anything that *generates* Phel, not only `.phel` files
- Every class outside the public PHP surface now carries `@internal`, so the public/internal split defined in [the stability policy](docs/stability.md) reaches IDEs and static analysis instead of living only in prose. A test pins both directions: no internal class without the annotation, no public class with it
- macOS is now a supported platform rather than an assumption: the compiler, core and PHAR suites run on `macos-latest` in CI alongside Linux, and a failure on either blocks a release. Windows runs a reduced suite as a declared best-effort tier. Every CI job used to be `ubuntu-latest`, on a project distributing a PHAR that people run everywhere
- Coverage is measured, published and gated. `composer test-compiler:coverage` has always worked locally, but every CI job installed PHP with `coverage: none`, so no figure existed and nothing enforced one. A new job publishes the percentage to the run summary and fails below a floor, currently 86.90% against a floor of 85
- Benchmarks now compare instead of only running. The bench job measured the pull request and printed a table with nothing to compare it against, so a regression was invisible. It now measures the base revision first, stores it as `baseline`, and asserts the pull request against it
- Mutation testing over `src/php/Lang/` and the analyzer, weekly and on demand (`infection.json5`, `composer infection`). Line coverage says a line ran; it does not say a test would have failed had that line been wrong. `Cons::walkTail()` passes the whole unit suite with its body disabled, which is exactly what a surviving mutant reports. Currently MSI 83% against a floor of 80
...
- The nREPL server now reports the session's current namespace in the `ns` field of eval responses. Evaluating `(ns foo)` or `(in-ns 'foo)` updated `*ns*` but the response kept saying `user`, so editor clients that drive their prompt from that field (CIDER, Calva, ...) never moved off `user>`. The session namespace is synced from the compiler's global environment after each eval, the same source of truth the `phel repl` prompt reads
- The nREPL server no longer leaves connecting clients with an unset (`nil>`) prompt namespace. Editors prime their namespace from the first eval response on connect: an empty init eval (`cider-repl-init-code ""`) was rejected as a missing `code` param, and CIDER's default Clojure-only init code fails to evaluate — both replies carried no `ns`. Empty code is now a no-op replying `done` with the current namespace (matching the reference nREPL), a missing `code` param gets the protocol's `no-code` status, and every eval error frame reports the current namespace
- A namespace's own `def`/`defn` now wins over a `:refer` of the same name, and defining a name that is already referred prints a warning naming both vars. The referred name used to win unconditionally and silently, so `(defn greet [] "mine")` in a namespace that also `:refer`s `greet` was reachable only as `my.ns/greet`; the bare name kept calling the other one. The worse variant was inside the definition itself: a recursive self-call resolved to the refer, so `(defn f [n] (if (= n 0) "base" (f (dec n))))` never recursed. It hit the prompt too, where the 14 injected `phel.repl` names (`doc`, `require`, `use`, `source`, ...) are exactly the words someone defines at a REPL: `(def doc 1)` then `doc` printed `<function:doc>`. A refer the namespace never redefines still resolves as before, in `phel repl`, the nREPL server and compiled code alike (#2897)
- A `(:require ...)` that resolves to no source file now fails at the require, naming both namespaces: `Cannot find namespace 'demoo' required by 'demo.test'`. It used to load nothing and say nothing, so the program ran on until the first use of a definition that was never loaded and reported `Cannot resolve symbol` several lines away from the `:require` that actually failed. This is how #2886 presented, and why it read as a `:refer` problem. `phel.repl/require` has always checked this; the `ns` form did not. Three things still resolve to nothing legitimately and are left alone: a namespace that is already loaded and so has no file (`(ns a)` then `(ns b (:require a))` at a prompt), a build (which resolves its dependencies itself), and the `phel.*`/`clojure.*` space, which resolves at runtime whether or not a source scan can see it (#2891)
- Re-defining a symbol no longer raises `DuplicateDefinitionException` under `phel eval` or the nREPL server. `(def a 1) (def a 2)` is ordinary at the REPL prompt and was an error on the very same input everywhere else, because the guard stood down only for `*repl-mode*`, which only `phel repl` sets. All three interactive entry points now set `*interactive-mode*`, and that is what the guard reads; compiling a file (`phel run`, `phel build`, `phel test`) still rejects a symbol defined twice (#2896)
...
- `seq` returns `nil` for an empty non-countable iterable, as its docstring promises
- `(lazy-seq 5)` reports `Don't know how to create a seq from: int` instead of leaking a PHP `TypeError`
- `((transient (sorted-set :a)) :a)` works, matching the transient hash set
- Sorted comparators are typed `bool|int`, so `(sorted-map-by < ...)` matches its contract (Phel's `<` returns a boolean)
- `phel\repl/compile-str` no longer throws on every call, and compiles in expression context without evaluating, so `(compile-str "(+ 1 2)")` returns `"3"`

- Kebab-case namespaces are no longer re-evaluated on every `(:require ...)` in the REPL, nREPL and `phel eval`, which re-ran their top level and any state it set up
- Syntax deprecation notices (bare `#`, `#| |#`, `|()`, `,`/`,@`, `$` gensym) were suppressed with `@` and never printed at all. `--warn-deprecations` now reports them, and stays silent for phel's bundled sources
- `--warn-deprecations` no longer blames your file for a deprecation inside someone else's macro. Calling `(delay ...)` reported `\Phel\Lang\Delay` at your own line; notices now name the file defining the macro and add the call site they were reached through
- `--warn-deprecations` no longer breaks the build on a `^:reference` param. The notice was written into the emitter's output buffer and spliced into the generated PHP. Compiler notices are now pinned to stderr, so no diagnostic can corrupt program output

- `phel lint` no longer aborts with `Symbol ... is already bound` on any project whose files `:require` one another. Re-reading a source during analysis is no longer mistaken for a redefinition, and a `defstruct` implementing a same-file `definterface` no longer crashes the run
- `phel/shadowed-binding` and `phel/unused-binding` now understand `for`/`dofor`/`foreach` heads, so a collection expression is no longer read as a bound name. `foreach` was in neither rule's list
- `phel/shadowed-binding` no longer flags a multi-arity `defn` for reusing a parameter name across arities, nor a metadata `fn` for reusing the enclosing one's
...
- A circular namespace dependency reports the whole cycle (`a -> b -> a`), not just where the walk re-entered
- Clearer messages for `--parallel 0`, odd-length map literals, and namespace-extraction parse failures, which now chain the original exception
- Parser, reader and analyzer errors chain the underlying exception as `getPrevious()`. The located message is unchanged
- Reporting an error no longer crashes while reporting: an unterminated list whose stream ends without an EOF token, and a code snippet with no significant token to anchor on, both used to replace the real diagnostic with a PHP `Error`

- Corrected 34 `:example` blocks whose documented output did not match what Phel prints, verified by evaluating every example. Sets print `#{…}`; float-returning fns print `2.0` not `2`; `subseq`/`rsubseq`/`repeatedly` return `@[…]`; quasiquote namespace-qualifies; `phel\http` structs print fully qualified; `queue` prints `<-(1 2 3)-<`; `test/get-stats` shows its `:skipped` bucket
- `phel\walk/postwalk`'s example no longer throws
- Documented 60+ previously-undocumented public core functions with `:example`/`:see-also`, and fixed a malformed `juxt` example

### Changed

- Cross-module dependencies are keyed by the interface the consumer asks for instead of a `FACADE_*` string constant. Gacela types `getProvidedDependency(X::class)` as `X`, so 31 hand-written `@var` docblocks and 30 constants are gone: the binding id *is* the type now, and a provider and its accessor can no longer disagree the way a `@var` assertion silently could. Each provider method also returns the interface it publishes rather than the concrete facade behind it
- `CrossModuleViaFacadeRule` is enabled. `.claude/rules/php.md` has required modules to reach each other only through a Facade for a long time, but only hand-rolled tests enforced parts of it, and those check Factories rather than Application classes. `Phel\Shared`, `Phel\Lang` and `Phel\Config` are declared shared kernels. The 35 existing violations are pinned with counts rather than waived, so a new cross-module reach fails the build

- Verified against `gacela-project/gacela` **2.0** (dev-main): it drops `AbstractDependencyProvider`, `GacelaConfig::addMappingInterface()`, `DocBlockResolverAwareTrait` and the internal dual provider-resolution path, and raises `gacela-project/container` to `^1.0` (where `Gacela\Container\Container` is `final` and composed rather than extended). Phel needed no source change for any of it: the removed APIs were already unused, every class resolving a pillar through `ServiceResolverAwareTrait` already declares it with `#[ServiceMap]`, and nothing type-hints the upstream concrete container. The constraint deliberately stays at `^1.21` until 2.0 is tagged; the bump is a one-line follow-up. A project embedding Phel that still uses those APIs must migrate on gacela 1.21 first, which emits a runtime notice for each
- gacela 2.0 also removes the PHPStan suppression for undeclared pillar accessors, so `getFacade()`/`getFactory()`/`getConfig()` are now *typed* from `#[ServiceMap]` instead of evaluating to `mixed`. That restores checking of everything reached through an accessor, and Phel stays at zero PHPStan errors under it

- **BREAKING** (installation): `symfony/console` narrows from `^6.0|^7.0|^8.0` to `^7.3|^8.0`. The old range was never true: `phel\cli` renders with the `markdown` output style, which arrived in Symfony 7.3, so a project resolving `6.4` or `7.2` got a working install and a failing `phel` command. `symfony/routing` has required `^7.3|^8.0` all along, so nothing that installs cleanly today is affected. Both ends of the new range now run the core and integration suites in CI
...
- **BREAKING** (PHP API): the five transfers named by `ApiFacadeInterface` moved from `Phel\Api\Transfer\` to `Phel\Shared\Api\`: `Diagnostic`, `ProjectIndex`, `Definition`, `Location`, `Completion`. Class shapes are unchanged. `Phel\Api\Transfer\` keeps the PHP-interop reflection types
- **BREAKING** (PHP API): the four unrelated interfaces named `FileIoInterface` are renamed after what each does: `Filesystem` → `DirectoryWritabilityCheckerInterface`, `Build` → `FileContentsIoInterface`, `Formatter` → `ValidatedFileIoInterface`, `Interop` → `FileWriterInterface`. Signatures unchanged, contracts not merged
...
- **BREAKING**: removed the deprecated reader syntax, part of [#2827](https://github.com/phel-lang/phel-lang/issues/2827): `#| ... |#` multiline comments, bare `#` line comments (use `;` / `;;`), the `|(...)` short fn with its `$`/`$1` params (use `#(...)` with `%`/`%1`), and `foo$` auto-gensym (use `foo#`). Those four now fail to lex or report an unresolvable `|`, so a file still using them stops loudly. **`,` is the exception and the one to grep for**: it is plain whitespace everywhere now, syntax-quote included, so `` `(foo ,x) `` still parses and quietly quotes `x` instead of unquoting it. `~` is unquote and `~@` unquote-splicing. Phel's own stdlib had three such commas (`aset`, `aset-in`, `router/compiled-router`). See [the migration guide](docs/migration/removed-deprecated-core-fns.md) for the grep. A trailing `$` is otherwise untouched: it stays the `:post`-condition return value and an ordinary character in a name
- **BREAKING**: removed the deprecated CLI option aliases `phel index --out` (use `--output` / `-o`) and `phel config --json` (use `--format=json` / `-f json`), part of [#2827](https://github.com/phel-lang/phel-lang/issues/2827). Passing a removed alias now fails with `The "--out" option does not exist.`, so a stale CI invocation surfaces immediately. `Phel\Shared\Console\DeprecatedOptionWarner` had no callers left and is removed with them. See [the migration guide](docs/migration/removed-deprecated-core-fns.md)
- **BREAKING**: removed the legacy REPL history migration. Since `0.37.0` every REPL start moved a top-level `.phel-repl-history` into `.phel/repl-history`; twelve minors on, that migration and its `PHEL_QUIET_MIGRATION` switch are gone. A project that upgraded through any release since `0.37.0` is unaffected. One jumping straight from before it keeps its old file untouched and starts a fresh history: `mkdir -p .phel && mv .phel-repl-history .phel/repl-history`
- **BREAKING**: removed `^:reference` as a by-reference fn-parameter marker; `^:by-ref` is the spelling, part of [#2827](https://github.com/phel-lang/phel-lang/issues/2827). The keyword is a string literal rather than a symbol, so grep the literal `:reference` in parameter metadata: a parameter still marked `^:reference` now compiles by value and silently stops propagating mutations to the caller
- **BREAKING**: removed `set-meta!` (use `with-meta`, deprecated since `0.32.0`) and `phel\test/print-summary` (`run-tests` already emits the `:summary` event, so calling it double-reported). A custom reporter reacts to the event; a harness that needs to force one builds it from the public `get-stats` snapshot. See [the migration guide](docs/migration/removed-deprecated-core-fns.md)
- **BREAKING**: removed long-deprecated core aliases (#2784): `push` (use `conj`), `put` (`assoc`), `unset` (`dissoc`), `put-in` (`assoc-in`), `unset-in` (`dissoc-in`), `values` (`vals`), `function?` (`fn?`), `hash-map?` (`map?`), `id` (`identical?`), `str-contains?` (`phel\string\contains?`). See [the migration guide](docs/migration/removed-deprecated-core-fns.md)
- **BREAKING** (PHP API): removed `ExceptionPrinterInterface::printError()` and `::printException()` with their `TextExceptionPrinter` implementations. Nothing called them, and both bypassed the `OutputInterface` every command writes through. Custom implementations can drop the methods
- Removed five facade methods documented as test support but used by nothing: `LspFacade::createDispatcher()`, `NreplFacade::createOpDispatcher()`, and `WatchFacade::createFileWatcherBuilder()`/`::createReloadOrchestrator()`/`::createNamespaceResolver()`. The `Factory` equivalents stay as internal wiring
- Removed unused internals: `LazySeqConfig::EAGER_THRESHOLD`, `NamespaceCacheInterface::remove()`/`::clear()`, `SessionRegistry::ids()`, and the empty `FiberProvider`
...
#### Reader & Compiler
- `#inst "..."` reads as `\DateTimeImmutable` and `#regex "..."` reads as a delimited PCRE string; user tags via `(register-tag "name" f)` in the new `phel\reader` namespace; `data-readers.phel` at any source root is auto-loaded (#1523)
- `#php` reader literal: `#php [1 2 3]` expands to `(php-indexed-array 1 2 3)`; `#php {"a" 1}` to `(php-associative-array "a" 1)` (non-recursive) (#1472)
- PHP interop shorthands: `(.method obj args)`, `(.-field obj)`, `(ClassName/method args)`, `\Ns\Class/MEMBER`, `(new ClassName args)` (#1467, #1471, #1477)
- `def` returns a printable var ref (e.g. `#'user/my-var`) (#1468)

#### REPL
- History vars `*1`, `*2`, `*3`, and `*e` for last exception (#1474)
- Prompt shows current namespace and tracks `(ns ...)` switches (#1474)

#### CLI
- `phel eval -` reads the expression from stdin (#1494)
- `phel agent-install [<platform>|--all]` writes skill files for Claude Code, Cursor, Codex, Gemini, Copilot, Aider; `--with-docs`, `--dry-run`, `--force` (#1495)
- `phel nrepl --port=N --host=addr` bencode-over-TCP nREPL server with `eval`, `clone`, `close`, `describe`, `load-file`, `interrupt`, `completions`, `lookup`, `info`, `eldoc` (#1507)
- `phel analyze <file>` prints JSON diagnostics; `phel index <dir>... [--out=file.json]` builds a symbol table; `phel api-daemon` serves the Api facade as JSON-RPC on stdio (#1508)
- `ApiFacade::analyzeSource`, `indexProject`, `resolveSymbol`, `findReferences`, `completeAtPoint` (#1508, #1518, #1525)
- `phel lint [paths]... [--format=human|json|github] [--config=path] [--no-cache]` with rules: unresolved-symbol, arity-mismatch, unused-binding, unused-require, unused-import, shadowed-binding, redundant-do, duplicate-key, invalid-destructuring, discouraged-var; configurable via `phel-lint.phel` (#1509)
- `phel watch [paths]... [-b backend] [--poll=500] [--debounce=100]` reloads changed namespaces in dependency order (inotify, fswatch, polling); `(watch! ["src/"])` via `phel\watch` (#1512)
- `phel lsp` LSP v3.17 server over stdio with hover, definition, references, completion, document/workspace symbols, rename, formatting, debounced publishDiagnostics (#1513)

#### Agent docs
- `.agents/` ships task recipes, per-platform adapters, and example projects (`todo-app`, `http-json-api`, `cli-wordcount`) (#1495)
- `composer test-agents` validates every example; runs in CI (#1500)

#### Formatter
- Aligns key/value pairs in `cond`, `case`, `condp`, and bindings of `let`/`loop`/`binding`/`for`/`foreach`/`dofor`/`if-let`/`when-let` (#1452)

#### Testing
- `phel\test/report` is a multimethod dispatching on event `:type` (#1516)
- Reporters: `default`, `testdox`, `dot`, `tap`, `junit-xml`; select via `phel test --reporter=<name>` (repeatable); `--output=path` for junit-xml (#1516)
- `phel test` selectors: `--include=<tag>`, `--exclude=<tag>`, `--ns=<glob>`, `--filter=<regex>` (repeatable); tag via `^:integration` or `^{:tags [:integration :slow]}`; skipped tests emit `:skipped` (#1517)
- `defspec` shrinks counterexamples via rose-tree `phel\test\shrink`; emits `:defspec-failed` with `:shrunk-args`, `:original-args`, `:shrink-steps`, `:seed`; `^:no-shrink` or `:shrink? false` opts out (#1519)

#### Modules
- `phel\test\gen`: generators, `sample`, `quick-check`, `defspec` with seedable PRNG (#1451)
- `phel\ai`: `chat-with-tools` OpenAI tool use, `tool-calls`, `tool-result`; exponential backoff on 429/5xx; per-call `opts` (`:provider`, `:timeout`, `:base-url`, `:api-key`, `:max-retries`); `*http-post*` seam; `docs/ai-guide.md` (#1456)
- `phel\core`: `uuid=`, `uuid-nil?`, `uuid-version`, `uuid-variant` (#1476)
- `phel\core`: `defmulti` accepts optional docstring `(defmulti name "doc" dispatch-fn)` (#1515)
- `phel\repl`: `find-ns`, `create-ns`, `remove-ns`, `intern`, `ns-interns` (#1488)
- `phel\cli`: spec-map wrapper over `symfony/console` with prompts, tables, progress, coercion, hooks, signals, test helpers; `docs/cli-guide.md` (#1498)
- `phel\match`: `match` macro with literal, vector, map, wildcard, `:as`, `:guard`, `:or`, rest-binding patterns (#1511)
- `phel\schema`: `validate`, `explain`, `conform`, `coerce`, `generate`, `instrument!`; kinds `:vector`, `:set`, `:map`, `:map-of`, `:tuple`, `:enum`, `:and`, `:or`, `:maybe`, `:re`, `:fn`, `:ref`, `[:=> args ret]`; named-schema registry (#1520)
- `phel\async`: fiber-backed `promise`, `deliver`, `future-call`, `future-fiber`, `future?`; 3-arg `deref` timeouts; `docs/async-guide.md` (#1521, #1522)
- `phel doc` and REPL completion cover `phel\async`, `phel\cli`, `phel\match`, `phel\pprint`, `phel\router`, `phel\walk`, `phel\test\gen` (#1518)

### Fixed

#### REPL & Compiler
- `eval()` runtime errors map to user `string:N` via source map (#1475)
- Non-callable literal calls (`('foo)`, `(42)`, `(nil)`, `("x")`) raise `PHEL011` at analysis time with source location (#1473)
- REPL multi-line buffer counts `#(...)`, `|(...)`, `#?(...)`, `#?@(...)`, `[]`, `{}`, `#{...}` toward balance (#1493)
...
- `phel\ai` text extraction picks first `text` block, skipping preceding `tool_use` (#1456)
- `phel\http/request-from-globals` error points to `request-from-map` for tests (#1524)
- `(:key nil)` returns default (#1504)
- `(get v nil)` and `(get l nil)` return default on vectors/lists (#1504)
- Vector/list with nil index raise `InvalidArgumentException` (#1504)
- Stack-trace arg rendering truncates each Phel argument at 200 chars (#1504)
- `phel test --reporter=junit-xml` preserves namespace and testcase order in output (#1528)

### Changed

- `docs/php-interop.md`: namespaced PHP functions (`php/Amp\trapSignal`) and `(def alias php/\Ns\fn)` capture (#1469)
- `phel build` prints summary with fresh/cached counts and output directory (#1455)
- `phel\ai/chat-with-tools` returns `{:text :tool-calls :stop-reason :raw}` (#1456)

### Removed

#### Public API
- `phel\http/create-response-from-map`, `phel\http/create-response-from-string` (use `response-from-map` / `response-from-string`) (#1491)
- `Keyword::createForNamespace()` (use `Keyword::create($name, $namespace)`) (#1485)
- `PhelConfig::setOut()` (use `PhelConfig::setBuildConfig()`) (#1485)
- `PhelBuildConfig::setMainPhpFilename()` (use `PhelBuildConfig::setMainPhpPath()`) (#1485)
- `PhelFunction` accessors `name()`, `doc()`, `fnSignatures()`, `signature()`, `description()`, `groupKey()`, `githubUrl()`, `docUrl()`, `file()`, `line()`, `namespace()` (use readonly properties) (#1485)

#### Internals
- `GlobalEnvironmentNotInitializedException`, `PhelFileFinder`, `PhelFileFinderInterface` (#1482, #1490)
- `FileException::canNotCreateTempFile()`, `ExtractorException::duplicateNamespace()`, `EmitterResult::getSource()`, `TokenStream::getReadTokens()`, `LoadClasspath::resetCache()` (#1482, #1490)

### Changed (breaking)

- `phel init` defaults to Flat layout (`src/`, `tests/`); pass `--nested` to keep the legacy `src/phel/<name>/` layout. Existing projects keep working since scaffolding only runs on new projects (#1453).
- `ProjectLayout::Conventional` renamed to `ProjectLayout::Nested`; `PhelConfig::useConventionalLayout()` renamed to `useNestedLayout()` (#1453).
