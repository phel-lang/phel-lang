...
- Qualified members work in value position: `\C/m` is a static method as a value (`(map \Phel\Lang\Keyword/create ["a" "b"])`) and `\C/.m` an instance method as a function of its receiver. `\C/CONST` still reads the constant and wins when a class has both, so `\C/new` is never a constructor (#2883)
- The dot syntax reaches a static method through a class name held in a value: `(.cases enum-class)` works whether it holds an object or `"\\App\\Status"` (#2881)
- `php-invoke` calls a method whose name is only known at runtime: `(php-invoke obj "format" "Y")`, the counterpart of ClojureScript's `js-invoke`. A class-name target calls the static method (#2887)
- `set!` covers both shapes Clojure's does: `(set! (.-total o) 10)` assigns a property, `(set! *x* 2)` assigns a dynamic var's current `binding` frame and throws when there is none. `alter-var-root` stays the way to change a root (#2884, #2888)
- The standard library is now written in the Clojure-style interop spelling, and `docs/` leads with it; `php/new`, `php/->` and `php/::` remain as the escape hatch (#2875, #2882)
- `phel nrepl` writes the bound port to a Clojure-standard `.nrepl-port` file for editor discovery, and removes it on exit; Ctrl+C and SIGTERM now shut the accept loop down gracefully (#2894)
- Deprecation warnings for your own code: a `def`/`defn` with `:deprecated` metadata warns at every call site under `--warn-deprecations` (or `PHEL_WARN_DEPRECATIONS=1`)
- `phel\test/with-isolated-stats` and `with-isolated-reporters` run a body against fresh statistics or reporters and restore the caller's afterwards, even when the body throws
- New lint rules: `phel/comment-style` (a whole-line comment written with a single `;`) and `phel/duplicate-def` (a symbol defined twice in one file)
- `phel agent-install --check` reports which skill files are installed and whether `.agents/` is behind the bundled docs, exiting 1 when it is
- `phel eval -h` shows a heredoc example for multi-line one-shot evaluation
- [Architecture decision records](docs/adr/README.md): one page per decision that shaped the repository, with the context, the cost, and the test that fails when it is broken. The first twelve cover the ones that look like oversights from the outside and get re-proposed: compiling to PHP source, the facade-only module boundary, the four accepted module cycles, the public API as a rule plus a snapshot, the single opt-in deprecation channel, Clojure-style interop as the only source spelling, the `.` separator keeping `\` alive, the precompiled stdlib in the PHAR, static analysis stopping at `src/`, persistent collections in PHP, and non-hygienic macros
- The normative specification: [language surface](docs/spec/language-surface.md) (reader syntax, the closed special-form list, the standard-library contract, the non-goals) and [Clojure divergences](docs/spec/clojure-divergences.md). A test compares the special-form table against the analyzer's dispatch registry, so the spec cannot drift from the compiler
- The [stability policy](docs/stability.md): what a version number promises, symbol by symbol, plus the 1.x deprecation and PHP-support policies. Migration guides for [0.49 to 1.0](docs/migration/upgrade-0.49-to-1.0.md), [the live deprecations](docs/migration/deprecated-surface.md) and [the removed core fns](docs/migration/removed-deprecated-core-fns.md)
- Compatibility gates that fail the build on drift: a snapshot of the public PHP surface (`composer api-surface:update`), one of every public `phel.*` definition and its arities (`composer core-api:update`), and a documentation gate requiring `:doc` on public definitions
- Every class outside the public PHP surface carries `@internal`, so the public/internal split reaches IDEs and static analysis. A test pins both directions
- CI: macOS is a supported platform rather than an assumption (Windows stays a declared best-effort tier), coverage is published and gated at 85%, benchmarks are asserted against the base revision instead of printed, and mutation testing runs weekly over `src/php/Lang/` and the analyzer (`composer infection`, MSI floor 80)

### Fixed

- Bare all-caps PHP classes such as `PDO` are no longer mistaken for global constants, so `PDO/ATTR_ERRMODE`, `(.-ATTR_ERRMODE PDO)` and `(new PDO ...)` need no leading backslash. If a class and global constant have the same name, the class wins; `php/NAME` remains the explicit constant escape hatch
...
- Re-defining a symbol no longer raises `DuplicateDefinitionException` under `phel eval` or the nREPL server; compiling a file still rejects it (#2896)
- Kebab-case namespaces are no longer re-evaluated on every `(:require ...)` in the REPL, nREPL and `phel eval`
- The nREPL server reports the session's namespace in the `ns` field, so editor prompts follow `(ns foo)` instead of sitting on `user>`, and a connecting client no longer lands on an unset `nil>` prompt: empty init code replies `done` with the namespace, a missing `code` param answers `no-code`, and error frames carry the namespace too (#2894)
...
- Syntax deprecation notices (bare `#`, `#| |#`, `|()`, `,`/`,@`, `$` gensym) are reported under `--warn-deprecations` instead of being suppressed outright, are attributed to the file defining the macro rather than your call site, and can no longer corrupt generated PHP: compiler notices are pinned to stderr
- `phel lint` no longer aborts with `Symbol ... is already bound` on projects whose files `:require` one another, and a rule that crashes is reported as `phel/internal-error` with a non-zero exit instead of vanishing from a clean-looking report
- Lint accuracy: `for`/`dofor`/`foreach` heads are understood by `phel/shadowed-binding` and `phel/unused-binding`, multi-arity `defn` parameters are not flagged as shadows, dot-separated `:require` aliases resolve, and `phel/discouraged-var` keys off real `:deprecated` metadata. Editor completion follows the same fixes
- An unreadable `phel-lint.phel`, source file or directory fails loudly instead of linting with defaults
- `php -d <ini>=<value> bin/phel …` no longer loses the setting: the OPcache re-exec rebuilt argv without the interpreter's own flags, so every `-d`/`-n`/`-c` was dropped
- CLI exit codes and messages: `phel format` exits non-zero on an unparsable file (so a `--dry-run` gate cannot pass over broken sources), `phel ns <unknown>` exits 1 instead of printing a `phel.core`-only listing, `phel doc <no-match>` says so instead of printing an empty table, and `phel config` ends with an error summary pointing at `phel doctor`
...
- `phel agent-install` syncs `.agents/` incrementally (#2809): a re-run writes only what changed upstream, leaves files you edited alone and names them, backs them up to `<file>.pre-phel.bak` under `--force`, and `--uninstall` removes only what it installed. A manifest at `.agents/.phel-agent-manifest.json` records the docs version and those paths
- Cross-module dependencies are keyed by the interface the consumer asks for rather than a `FACADE_*` constant, so 30 constants and 31 hand-written `@var` docblocks are gone and a provider cannot disagree with its accessor. `CrossModuleViaFacadeRule` is enabled with the 35 existing violations pinned by count, so a new cross-module reach fails the build
- Every compiler deprecation notice goes through one switch, one message factory and one dedup table
- Requires `gacela-project/gacela` `^1.21` (was `^1.18`), which types the `#[ServiceMap]` pillar accessors instead of suppressing them. Verified against gacela **2.0** (dev-main) with no source change needed; the constraint bump is a one-line follow-up once 2.0 is tagged

### Removed

- **BREAKING**: the deprecated reader syntax, part of [#2827](https://github.com/phel-lang/phel-lang/issues/2827): `#| ... |#` comments, bare `#` line comments (use `;`/`;;`), the `|(...)` short fn with `$`/`$1` (use `#(...)` with `%`/`%1`), and `foo$` auto-gensym (use `foo#`). Those fail to lex, so a stale file stops loudly. **`,` is the exception to grep for**: it is plain whitespace everywhere now, syntax-quote included, so `` `(foo ,x) `` still parses and quietly quotes `x` instead of unquoting it (`~` is unquote, `~@` unquote-splicing). See [the migration guide](docs/migration/removed-deprecated-core-fns.md)
- **BREAKING**: `^:reference` as a by-reference parameter marker; `^:by-ref` is the spelling ([#2827](https://github.com/phel-lang/phel-lang/issues/2827)). This one is also silent: a parameter still marked `^:reference` compiles by value and stops propagating mutations, so grep the literal `:reference` in parameter metadata
- **BREAKING**: long-deprecated core aliases (#2784): `push` (`conj`), `put` (`assoc`), `unset` (`dissoc`), `put-in` (`assoc-in`), `unset-in` (`dissoc-in`), `values` (`vals`), `function?` (`fn?`), `hash-map?` (`map?`), `id` (`identical?`), `str-contains?` (`phel\string\contains?`), plus `set-meta!` (use `with-meta`) and `phel\test/print-summary` (`run-tests` already emits the `:summary` event). See [the migration guide](docs/migration/removed-deprecated-core-fns.md)
- **BREAKING**: the deprecated CLI option aliases `phel index --out` (use `--output`/`-o`) and `phel config --json` (use `--format=json`), part of [#2827](https://github.com/phel-lang/phel-lang/issues/2827). A stale invocation now fails with `The "--out" option does not exist.`
