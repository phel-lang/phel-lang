...
- Requiring a namespace that resolves to no source file is no longer a silent no-op that exits 0: `(ns app (:require some.missing.ns))` (or any transitive `(:require ...)` of a resolved namespace) now fails fast with `Cannot find namespace 'some.missing.ns' required by 'app'` instead of letting the typo surface much later as a generic `Cannot resolve symbol`. `clojure.*` requires that remap to a bundled `phel.*`, already-loaded namespaces, and PHP-interop `(:use ...)` are unaffected (#2636)
