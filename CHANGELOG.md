...
- **BREAKING** (installation): requires `gacela-project/gacela` `^2.0` (was `^1.18`). An app pinned to gacela 1.x upgrades with it, and 1.21 is the final 1.x release. The migration is three renames Phel had already made, and `vendor/bin/gacela doctor` names the one that fails silently. Gacela 2.0 types the `#[ServiceMap]` pillar accessors instead of suppressing them, so a call reached through one is checked rather than reading as `mixed`
