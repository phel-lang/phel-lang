...
- (PHP API) `Phel\Api\Transfer\Definition` gains a `deprecated` field (the `:deprecated` reason, `''` when absent) plus `isDeprecated()`, and `toArray()` serializes it. The constructor argument is optional, so existing call sites keep working
