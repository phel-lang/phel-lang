...
- A global read of a var that is not `^:dynamic` goes straight to the registry root: the emitter writes `\Phel\Lang\Registry::readRoot(...)` where it used to write `\Phel::getDefinition(...)`, skipping a dynamic-scope gate that only a `^:dynamic` var can ever trip, since `binding` refuses anything else and `with-redefs`, `phel.mock` and `dotrace` all swap the root the read observes. The read is 1.6x faster with nothing bound and 1.9x faster while any `binding` frame is open, which is the steady state of `phel test` because `deftest` binds `*current-test-name*` around every test body; a benchmark doing four reads and a call is 15.9% faster end to end. `^:dynamic` and `^:redef` vars keep the full read path, and a var that will be redefined `^:dynamic` later in the same process (a REPL or a reload) needs one of those tags for already-compiled readers to observe its bindings (#3179)
- A map becomes a hash map at 4 entries, and every construction path agrees on where that is. `TypeFactory::persistentMapFromArray` compared the flat `[k1, v1, ...]` element count against `PersistentArrayMap::MAX_SIZE`, which counts entries, so a `{...}` literal promoted at half the constant while `assoc`, `into` and `zipmap` promoted at the constant: the same six-entry map read 3.4x slower when grown than when written as a literal. Literal behaviour is unchanged, because the constant moved to where the bug had already put the literals. Reading a grown five-to-eight entry map is 4.1x to 5.5x faster and a missing-key read 5.5x to 8.4x; building one costs a promotion earlier, and iterating one is 3.0x to 3.4x slower because it now walks the hash representation, as a literal of that size already did (#3172)
- A bare all-caps PHP name resolves by position instead of by probing the class table. In value position it is always the global constant (`PHP_EOL`, `JSON_PRETTY_PRINT`); as a member target, a constructor or a callable (`(WP_CLI/log "x")`, `(php/new PDO dsn)`, `(php/callable PDO getAvailableDrivers)`) it is always the class. Identical source used to compile to different PHP depending on which classes the compiling process had autoloaded: a class autoloadable only under WP-CLI made `(WP_CLI/log ...)` a direct static call there and a dynamic call through an undefined constant everywhere else, and the compiled-code cache then froze whichever ran first. Code that read a loadable all-caps class in value position now gets the constant, with a compile-time warning naming the spellings that reach the class (`\PDO`, `(:use PDO)`, `PDO/class`); `php/NAME` stays the explicit constant everywhere. Recorded as [ADR 0016](docs/adr/0016-a-bare-all-caps-host-name-reads-by-position.md) (#3064)
- `CompilerFacadeInterface` gains `emptyNodeEnvironment()` and `enableDeprecationWarnings()`, so Api, Build and Console stop importing `NodeEnvironment` and `DeprecationWarnings` out of the compiler's `Domain`. The compiler's internals are now reached only from inside the Compiler module, and the four architecture-rule exemptions those imports needed are gone. `WarnDeprecationsFlag::applyAndStrip()` is now `strip()`, which only filters argv; `ConsoleBootstrap` compares the two lists and asks the facade to flip the switch (#3048)
...
- `update` has Clojure's fixed arities, `[m k f]` through `[m k f x y z]` and then variadic. Every call built a rest argument and went through `apply`, even `(update m :k inc)` with no extra argument, which is nearly all of them: that call is 1.31x faster and `(update m :k + 1)` 1.57x. Four or more extra arguments keep the `apply` path. Behaviour is unchanged, including `f` receiving `nil` for an absent key and a `nil` map behaving as an empty one (#2973)
- `bit-and`, `bit-or`, `bit-xor` and `bit-and-not` have fixed two- and three-argument arities. A direct call to the first three expands through `:inline` and was already free, but `apply` and every higher-order use (`(reduce bit-or 0 flags)`) reached a body that built a rest argument, `concat`ed it, walked it for nil and reduced with a closure on every call; `bit-and-not` has no `:inline`, so even a direct call paid all of that. `(apply bit-and [12 10])` is 14x faster, `(bit-and-not 255 15)` 22x, `(reduce bit-or 0 flags)` over eight flags 14x. Nil is still rejected on every arity, and a call with four or more arguments keeps the old body (#2973)
- `intersection`, `difference` and `symmetric-difference` have fixed arities for the two-set call, which is nearly every call. Each built a rest argument and reduced over it before doing any set work: two sets are 1.21x faster for `intersection`, 1.22x for `difference` and 1.30x for `symmetric-difference`, which also stops allocating its pairing closure per call. Behaviour is unchanged, including the one-argument call returning its argument as it stands and the no-argument call still being an arity error (#2973)
...
- `phel test` and `phel bench` on a file outside the configured `src-dirs`/`test-dirs` load what it requires before loading it. The dependency walk indexes only the configured directories, so seeding the outside file's own namespace dequeued a name with no entry and dropped its requires; a cold run still passed because compiling the file pulled them in, but a warm run evaluated the cached artifact first and died at the first cross-namespace call, then evicted the entry, so the runs alternated between passing and failing. The requires the file declares are now seeded into the walk and load in order on every run (#3187)
