...
- Coverage is measured, published and gated. `composer test-compiler:coverage` has always worked locally, but every CI job installed PHP with `coverage: none`, so no figure existed and nothing enforced one. A new job publishes the percentage to the run summary and fails below a floor
- Benchmarks now compare instead of only running. The bench job measured the pull request and printed a table with nothing to compare it against, so a regression was invisible. It now measures the base revision first, stores it as `baseline`, and asserts the pull request against it using the tolerance already configured in `phpbench.json`
