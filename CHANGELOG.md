...
- Architecture: promoted two implicit cross-module contracts to their proper homes (pure relocations, behavior unchanged) (#2263): the parse-tree value objects (`Compiler\Domain\Parser\ParserNode\*` and the lexer `Token`) moved to `Phel\Shared\Parser\Node\*` (consumed as an AST contract by Formatter/Lint/Api while Compiler still produces them; removes the `CompilerFacadeInterface -> Compiler\Domain` parse-tree references), and the stateful `LoadClasspath` moved from `Compiler\Domain\Analyzer\Resolver` to `Phel\Lang`, next to the `Registry` slot it owns
- Compiler internals: split the 882-LOC `ConstantFolder` into a thin dispatcher plus `LiteralArithmeticFolder`, `LiteralCollectionFolder`, and `LiteralBitwiseFolder`, and extracted `FnSymbol`'s `{:pre :post}` assert rewriting into `FnPrePostConditionRewriter` (pure refactor, fold routing and assert expansion unchanged) (#2264)
